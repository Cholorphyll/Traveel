...
{
    /**
     * Make an API request with retry logic for handling rate limits and search status
     * 
     * @param string $url The API endpoint URL
     * @param int $maxRetries Maximum number of retry attempts
     * @param int $initialDelay Initial delay between retries in seconds
     * @return mixed API response data or null on failure
     */
    private function makeApiRequestWithRetry(string $url, int $maxRetries = 5, int $initialDelay = 2)
    {
        $retryCount = 0;
        ...
        do {
            try {
                ...
                if ($now < $rateLimitReset) {
                    sleep(($rateLimitReset - $now) + 1); // Add 1 second buffer
                }

                $response = Http::withHeaders([
                    'Accept' => 'application/json',
                    'User-Agent' => 'TravellApp/1.0'
                ])->timeout(30)->get($url);
                
                // Handle rate limiting (429)
                if ($response->status() === 429) {
                    $retryAfter = (int) ($response->header('Retry-After') ?? 60);
                    $rateLimitReset = time() + $retryAfter;
                    throw new \Exception("Rate limit exceeded");
                }
                
                // If search is not finished (409)
                if ($response->status() === 409) {
                    throw new \Exception('Search is not finished');
                }
                
                // If we get a successful response, return the JSON data
                ...
                $delay = ($baseDelay * (2 ** ($retryCount - 1))) + (mt_rand(1, 1000) / 1000);
                
                // If we have a rate limit reset time, use that instead
                if ($rateLimitReset > time()) {
                    $delay = max($delay, $rateLimitReset - time() + 1);
                }
    ...
    /**
     * Get nearby airports based on hotel coordinates
     * 
     * @param float $lat Hotel latitude
     * @param float $lon Hotel longitude
     * @param int $limit Maximum number of airports to return
     * @return \Illuminate\Support\Collection
     */
    private function getNearbyAirports($lat, $lon, $limit = 3)
    {
        // Convert coordinates to float to ensure proper calculation
        $lat = (float) $lat;
        $lon = (float) $lon;
    ...
    if(!empty($searchresults)){
       ...
       $images = $this->makeApiRequestWithRetry($url);
       if ($images === null) {
...
//  return $locid;
    ...
    // Get nearby airports if hotel has coordinates
    $nearbyAirports = collect();
    if (!empty($searchresults) && isset($searchresults[0]->Latitude) && isset($searchresults[0]->longnitude)) {
        $nearbyAirports = $this->getNearbyAirports($searchresults[0]->Latitude, $searchresults[0]->longnitude);
    }
    ...
    if ($locationData) {
       ...
       //start price code
        ...
        $transportationResponse = $this->getNearbyTransportation($transportationRequest);
        
        // Convert JsonResponse to array if it's a JsonResponse object
        $nearbyTransportation = [];
        if ($transportationResponse instanceof \Illuminate\Http\JsonResponse) {
            $nearbyTransportation = json_decode($transportationResponse->getContent(), true);
        } elseif (is_array($transportationResponse)) {
            $nearbyTransportation = $transportationResponse;
        }
       ...
       $nearbyattractions = $this->getNearbySights($location_slugid);
        ...
        //end price code
 // Get Question & Answer data from HotelQuestion table with caching
                // Clear the cache for this specific hotel's FAQs to ensure fresh data
         Cache::forget("hotel_{$searchresults[0]->id}_faqs");
        
        // Get Question & Answer data directly without caching for debugging
        try {
                    
            $allFaqs = DB::table('HotelQuestionAnswer')
                ->where('TPHotel_id', $searchresults[0]->id)
                ->select(
                    'HotelQuestionId', 
                    'Question', 
                    'Answer', 
            ...
            // Process the FAQs - no need to map fields since we already selected them with aliases
            $processedFaqs = $allFaqs->map(function ($faq) {
                // Derive a category for grouping from tags if available; fallback to 'hotel'
                if (!empty($faq->Tags)) {
                    // Use first tag as category if comma-separated
                    $parts = explode(',', $faq->Tags);
                    $faq->category = trim($parts[0]) !== '' ? trim($parts[0]) : 'hotel';
                } else {
                    $faq->category = 'hotel';
                }
                
                // Ensure Likes is set to avoid undefined property errors
                if (!isset($faq->Likes)) {
                    $faq->Likes = 0;
                }
            ...
            $hotelQuestionAnswer = [
                'all_faqs' => $processedFaqs,
                'grouped_faqs' => $processedFaqs->groupBy('category')
            ];
       ...
       ->with('nearbyAirports', $nearbyAirports)
       ->with('hotelQuestionAnswer', $hotelQuestionAnswer)
    ...
    /**
     * Submit a new question for a hotel
     *
    ...
    public function submitQuestion(Request $request)
    {
        ...
        try {
            ...
            // Insert the question into the HotelQuestion table
            $questionId = DB::table('HotelQuestion')->insertGetId([
                'HotelId' => $request->input('hotelId'),
                'Question' => $request->input('question'),
                'Answer' => null, // Will be answered by hotel staff later
                'IsActive' => 1, // Set to inactive until approved/answered
                'CreatedDate' => now(),
                'User_Name' => $userName,
                'Listing' => 'hotel', // Default category
            ]);
            
            // Clear the cache for this hotel's FAQs
            Cache::forget("hotel_{$request->input('hotelId')}_faqs");
            
            return response()->json([
                'success' => true,
                'message' => 'Your question has been submitted successfully and will be reviewed by the hotel.',
    ...
    /**
     * Update FAQ vote (like/dislike)
     *
     * @param Request $request
     * @return \Illuminate\Http\JsonResponse
     */
    public function updateFaqVote(Request $request)
    {
        try {
            $validator = Validator::make($request->all(), [
            ...
            // Get the current FAQ
            $faq = DB::table('HotelQuestion')->where('hotelQuestionId', $faqId)->first();
            ...
            // Check if the Likes column exists in the database
            $columns = DB::getSchemaBuilder()->getColumnListing('HotelQuestion');
            $hasLikesColumn = in_array('Likes', $columns);
            
            // If column doesn't exist, add it
            if (!$hasLikesColumn) {
                DB::statement('ALTER TABLE HotelQuestion ADD COLUMN Likes INT DEFAULT 0');
            }
            
            // Update the like count based on action
            $currentLikes = $faq->Likes ?? 0;
            ...
            if ($action === 'like') {
                ...
                // Decrease likes by 1 (dislike)
                $newLikes = max(0, $currentLikes - 1); // Ensure likes don't go below 0
                
                
                $message = 'Dislike recorded successfully';
            }
            
            // Update with the correct primary key column name
            DB::table('HotelQuestion')
                ->where('hotelQuestionId', $faqId)
                ->update([
                    'Likes' => $newLikes,
                    'updatedOn' => DB::raw('NOW()') // Use DB::raw to avoid Carbon date formatting issues
                ]);
    ...
    /**
     * Search FAQs by query term in questions, answers, and tags
     * 
     * @param Request $request
     * @return \Illuminate\Http\JsonResponse
     */
    public function searchFaqs(Request $request)
    {
        ...
        if ($validator->fails()) {
            return response()->json(['success' => false, 'message' => 'Invalid input', 'errors' => $validator->errors()], 400);
        }
        ...
        try {
            ...
            $columns = DB::getSchemaBuilder()->getColumnListing('HotelQuestion');
            $hasTagsColumn = in_array('Tags', $columns);
            
            // If Tags column doesn't exist, add it
            if (!$hasTagsColumn) {
                DB::statement('ALTER TABLE HotelQuestion ADD COLUMN Tags VARCHAR(255) DEFAULT NULL');
            }
            
            // Search in Question, Answer, and Tags fields
            $faqs = DB::table('HotelQuestion')
                ->where('HotelId', $hotelId)
                ->where('IsActive', 1)
