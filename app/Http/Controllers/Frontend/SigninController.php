 ...
 public function register_user(Request $request)
    {   
        ...
        if ($request->has('redirect_url')) {
            session(['registration_redirect' => $request->redirect_url]);
        }
       ...
       $userId = DB::table('FrontendUserLogin')->insertGetId([
        ...
        // If city_id is provided, update the user's location
        if ($request->has('city_id') && !empty($request->city_id)) {
            DB::table('FrontendUserLogin')
                ->where('UserId', $userId)
                ->update(['LocationId' => $request->city_id]);
        }
       ...
       // Send verification email
        ...
        try {
            // Attempt to send the email using the verify template
            Mail::send('emails.verify', ['user' => $user, 'token' => $verification_token], function ($message) use ($to, $subject) {
                $message->to($to)
                    ->subject($subject);
            });
            return 1;           
	 ...
	 public function user_login(Request $request){
        ...
        // Store the previous URL in session for redirection after login
        if (!$request->session()->has('url.intended')) {
            // Only store the URL if it's not already stored and it's not a login/signup page
            $previousUrl = url()->previous();
            $currentUrl = url()->current();
            
            // Don't store login/signup URLs as redirect targets
            if (!str_contains($previousUrl, 'user_login') && 
        ...
        if (session()->has('frontend_user')) {
            ...
            return view('front_login.user_login')->with([
            'type' => $type,
            'lname' => $lname,
            'checkin' => $checkin,
	...
	public function varification(Request $request)
	{
		...
		if ($user) {
            ...
            // Get the redirect URL from session if it exists
            $redirectUrl = '/';
            if (session()->has('registration_redirect')) {
                $redirectUrl = session('registration_redirect');
                // Clear the redirect URL from session
                session()->forget('registration_redirect');
            }
            
            // Add the modal parameter to the redirect URL
            $redirectUrl = $this->addModalParameterToUrl($redirectUrl, 'signInModal');
            
            // Pass both success message and verified email to the session
            return redirect($redirectUrl)
                ->with('success', 'Email verification successful. You can now login.')
	...
	/**
     * Helper function to add modal parameter to URL
     */
    private function addModalParameterToUrl($url, $modalId)
    {
...
public function userlongin(Request $request)
{
    ...
    if ($user && !empty($password) && Hash::check($password, $user->Password)) {
        ...
        if ($user->IsEmailVerify == 1) {
            ...
            // Store the current URL in session if provided and not already stored
            if ($redirectUrl && !session()->has('url.intended')) {
                session(['url.intended' => $redirectUrl]);
            }
            
            // Determine the redirect URL
            // Use the provided redirect URL if available, otherwise use the intended URL from session or default to dashboard
            $redirectTo = $redirectUrl ?? session('url.intended', route('user_dashboard'));
            ...
            return response()->json([
                ...
                'redirectUrl' => $redirectTo // Redirect to previous page or dashboard
            ]);
        } else {
            // Email not verified
            return response()->json([
...
public function front_dashboard()
{
    ...
    // $email = $userData['email'];
       ...
       ->select('u.*')
        ...
        return view('front_login.user_dashboard', [
    'getdata' => $getdata,
    'getpost' => $getpost,
...
{
   // Get the previous URL before logging out
    $previousUrl = url()->previous();

    // Log out any Laravel-authenticated user (e.g., Google/Socialite or default auth)
    try {
        \Illuminate\Support\Facades\Auth::logout();
    } catch (\Throwable $e) { /* ignore */ }

    // Clear custom frontend session user
    ...
    // Redirect back to the previous page (fallback to homepage if invalid)
    return redirect(filter_var($previousUrl, FILTER_VALIDATE_URL) ? $previousUrl : route('homepage'));
}
...
//end new code 
    ...
    public function addpost(Request $request)
    {
        ...
        $userid = $request->get('userid');
          ...
          // If user ID is not in request, try to get it from session
    if (empty($userid) && session()->has('frontend_user')) {
        $userData = session('frontend_user');
        $userid = $userData['UserId'];
    }
    
    // Convert empty string to 'unknown' for logging purposes
    $logUserId = !empty($userid) ? $userid : 'unknown';
...
{
    // Try to get user ID from request first
    $userid = $request->get('userid');
...
if ($request->hasFile('pimage')) {
    ...
    try {
        ...
        $imageUrl = $s3->getObjectUrl('s3-travell', 'user-images/' . $filename);
