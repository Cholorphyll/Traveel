   ...
   public function hotel_list($segment, Request $request)
    ...
    if (strpos($currentUrl, $baseCorrectUrl) === false) {
        ...
        foreach ($allParts as $part) {
            ...
            if (preg_match('/^(st\d+|rs\d+|price\d+|a[a-z]+\d+|n[a-z]+\d+|sqx\d+)/', $part) || 
                in_array($part, ['free_cancellation', 'parking', 'Internet', 'breakfast']) ||
                (isset($processed_amenities[$part]) || isset($processed_neighborhoods[$part]) || isset($processed_sights[$part]))) {
                $filterParts[] = $part;
            } 
            ...
            if( $chkin !="" && $checout !=""){
             ...
             // Add this at the start of your function to initialize
                ...
                try {
                    ...
                    $searchresults = Cache::remember($cacheKey, 1800, function() use ($slgid, $st, $amenity, $price, $reviewscore, $amenity_ids, $neighborhood_ids) {
        ...
        if($amenity == "") {
            $query = DB::table('TPHotel as h')
                ...
                ->select([
                    ...
                    'h.OverviewShortDesc','h.NeighborhoodId','h.Neighborhood',
