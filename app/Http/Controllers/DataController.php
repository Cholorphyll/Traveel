...
class DataController extends Controller
{
	public function formatDate($date) {
        ...
        // Normalize separators and remove extra symbols like arrows
        ...
        $formats = [
            ...
            'd m Y', 'm d Y', 'Y m d', 'Y M d', 'Y F d',
        ];

        foreach ($formats as $fmt) {
            try {
                return Carbon::createFromFormat($fmt, $normalized)->format('Y-m-d');
            } catch (\Throwable $e) {
                // try next format
            }
        }

        // Fallback: more permissive parser
        try {
    ...
    public function listLocation(Request $request)
    {
        ...
        $cachedData = Cache::remember($cacheKey, $cacheDuration, function () use ($originalSearchText) {
            ...
            foreach ($searchTexts as $searchText) {
                ...
                $searchParams = [
                    'limit' => 5,
                    'attributesToRetrieve' => ['id', 'name', 'slug', 'slugid', 'parentName', 'countryName', 'tourismScore'],
                    'sort' => ['tourismScore:desc']
                ];
       ...
       // Only save to search history if we have valid results and it's not a "Result not found" message
        if (!$locations->isEmpty() && isset($result[0]['id']) && $result[0]['value'] !== "Result not found") {
            ...
            try {
                ...
                $cookieMinutes = 2628000;
                Cookie::queue(Cookie::make('lastsearch', json_encode($searchHistory), $cookieMinutes, '/', null, $request->isSecure(), true, false, 'lax'));
            }  catch (\Exception $e) {
  ...
  public function recenthistory(Request $request)
    {
       $sight = 0;
        ...
        // Check both session keys for search history
        $sessionKeys = ['lastsearch', 'recent_sightlist_searches', 'recent_hotellist_searches'];
        $allSearches = [];

        try {
            $cookieLastSearch = $request->cookie('lastsearch');
            if (!empty($cookieLastSearch)) {
                $decoded = json_decode($cookieLastSearch, true);
                if (is_array($decoded)) {
                    $allSearches = array_merge($allSearches, $decoded);
                }
            }
        } catch (\Throwable $e) {
        }
        ...
        foreach ($sessionKeys as $key) {
            if (Session::has($key)) {
                ...
                $searches = is_string($data) ? @unserialize($data) : $data;
                if (is_array($searches)) {
                    $allSearches = array_merge($allSearches, $searches);
                }
            }
        }
        
        // Process and deduplicate searches
        if (!empty($allSearches)) {
            $uniqueSearches = [];
            $seen = [];
            
            // Process in reverse to get most recent first
            foreach (array_reverse($allSearches) as $search) {
                if (!is_array($search)) continue;
                
                $key = ($search['id'] ?? '') . '_' . ($search['val'] ?? $search['Name'] ?? '');
                ...
                $item = [
                    'id' => $search['id'] ?? '',
                    'Slug' => $search['key'] ?? '',
                    'value' => $search['Name'] ?? ($search['val'] ?? '')
                ];
                
                if (isset($search['type']) && $search['type'] === 'sight') {
                    $item['type'] = 'sight';
                    $sight = 1;
                    if (isset($search['SightId'])) {
                        $item['SightId'] = $search['SightId'];
                    }
                }
                
                $uniqueSearches[] = $item;
                
                // Limit to 5 most recent unique searches
                if (count($uniqueSearches) >= 5) {
                    break;
                }
            }
            
            $result = $uniqueSearches;
        }
...
public function explore(Request $request, $id)
{
    ...
    $searchresults = DB::table('Sight')
        ...
        ->select([
            ...
            'Sight.best_for', 'Sight.best_season', 'Sight.best_time_to_visit',
            'Sight.accessibility', 'Sight.kid_friendly', 'Sight.pet_friendly',
            'Sight.food_facilities', 'Sight.public_restrooms', 'Sight.parking_facilities',
            'Sight.packing_checklist', 'Sight.what_people_like', 'Sight.what_people_dont_like',
            'Sight.what_to_do', 'Sight.what_not_to_do'
        ])
    ...
    $sightreviews = DB::table('SightReviews')
        ->leftJoin('FrontendUserLogin', 'FrontendUserLogin.UserId', '=', 'SightReviews.UserId')
        ->whereNotNull('FrontendUserLogin.FirstName')
        ->select([
            'FrontendUserLogin.FirstName', 
            'SightReviews.ReviewDescription', 
            'SightReviews.IsRecommend', 
            'SightReviews.ReviewRating', 
            'SightReviews.CreatedDate',
            'FrontendUserLogin.Email',
            'FrontendUserLogin.st_profilelink as profile_image'
        ])
        ->where('SightReviews.SightId', $sighid)
        ->limit(20)
        ->get();
...
//                 });
                    ...
                    $searchresults = Cache::remember($cacheKey, 1800, function() use ($slgid, $st, $amenity, $price, $reviewscore, $amenity_ids, $neighborhood_ids, $propertyType_ids) {
                        $query = DB::table('TPHotel as h')
                            ...
                            if (!empty($amenity_ids)) {
                                $query->whereHas('amenities', function ($query) use ($amenity_ids) {
                                    $query->whereIn('amenity_id', $amenity_ids);
                                });
                            }
    ...
    ->first();
            ...
            return view('hotel_listing')
                ...
                // ->with('neabyhotelwithswimingpool', $neabyhotelwithswimingpool ?? collect([]))
...
$query->where('h.slugid', $desiredId)
    ->whereNotNull('h.slugid');
            ...
            if (!empty($amenity_ids)) {
                $query->where(function($q) use ($amenity_ids) {
                    foreach ($amenity_ids as $aid) {
                        $q->whereRaw('FIND_IN_SET(?, h.facilities)', [$aid]);
                    }
                });
            }
		...
		$neighborhoodIds = !empty($neghborhood_info) ? array_column($neghborhood_info, 'NeighborhoodId') : [];

        if (!empty($neighborhoodIds)) {
            $query->whereIn('h.NeighborhoodId', $neighborhoodIds);
        }
    ...
    ->first();
        ...
        // Get nearby hotels from NearByHotel table
        $nearbyHotelIds = DB::table('NearByHotel')
            ->where('slugid', $desiredId)
            ->pluck('hotelid')
            ->toArray();
        ...
        if (!empty($nearbyHotelIds)) {
            // Build query for nearby hotels with same structure as main query
            $nearbyQuery = DB::table('TPHotel as h')
                ->leftJoin('TPHotel_amenities as a', DB::raw('FIND_IN_SET(a.id, h.shortFacilities)'), '>', DB::raw('0'))
                ->select([
                    'h.hotelid', 'h.id', 'h.name', 'h.slug', 'h.stars', 'h.pricefrom',
                    'h.rating', 'h.amenities', 'h.distance', 'h.image', 'h.about',
                ...
                ->whereIn('h.hotelid', $nearbyHotelIds);

            // Apply same filters as main query
            if ($st != "") {
                $nearbyQuery->where('h.stars', $st);
            }
            if ($reviewscore != "") {
                $nearbyQuery->whereNotNull('h.rating')->where('h.rating', '>=', $reviewscore);
            }
            if (!empty($price)) {
                $nearbyQuery->where('h.pricefrom', '<=', (int)trim($price));
            }

            // Apply amenity filters
            if (!empty($amenity_ids)) {
                $nearbyQuery->where(function($q) use ($amenity_ids) {
                    foreach ($amenity_ids as $amenityId) {
                        $q->orWhereRaw('FIND_IN_SET(?, h.shortFacilities)', [$amenityId]);
                    }
                });
            }
            ...
            if (!empty($neghborhood_info)) {
                $neighborhoodIds = array_column($neghborhood_info, 'NeighborhoodId');
                $neighborhoodIds = array_filter($neighborhoodIds);
                if (!empty($neighborhoodIds)) {
                    $nearbyQuery->whereIn('h.NeighborhoodId', $neighborhoodIds);
                }
            }

            // Apply property type filters
            if (!empty($propertyType_ids)) {
                $nearbyQuery->whereIn('h.propertyType', $propertyType_ids);
            }

            $nearbyQuery->groupBy('h.id')
                ->orderBy('h.stars', 'desc');

            $nearbyHotels = $nearbyQuery->get();
            $nearbyCount = $nearbyHotels->count();
        }

        // Merge nearby hotels with main results
        $mainHotels = $searchresults->getCollection();
        $mergedHotels = $mainHotels->merge($nearbyHotels);
        
        // Update the paginated results with merged data
        $searchresults->setCollection($mergedHotels);
        
        // Update total count to include nearby hotels
        $total_count = $count_result + $nearbyCount;

        return view('frontend.hotel.hoteldata_withoutdate', [
            'count_result' => $total_count,
            'searchresults' => $searchresults,
            'nearby_hotels_count' => $nearbyCount,
            'lname' => $lname,
            'st' => $st,
...
//           $table = 'LIQNearby';
    ...
    public function hoteldetailnearbyrest(request $request){
        ...
        $nearby_hotel =  DB::table('TPNearby_hotel')->where('hid',$hid)->get()->unique('hotelid')->values();
 ...
 public function insert_hotel_desction(Request $request){
    try {
    $timeout = 8; // finite timeout to avoid upstream/proxy 502 due to long hang
 //   $checkinDate = date('Y-m-d');

    $checkin = $request->get('checkin');
    $checkout = $request->get('checkout');
    $hid = $request->get('hid');
    // Guard: required hotel id
    if (empty($hid)) {
        return response()->json(['status' => 'error', 'message' => 'Missing required parameter: hid'], 400);
    }
    $chkin = $checkin;
    $checout = $checkout;       
    ...
    if (!$checkin || !$checkout) {
        ...
        $checkout = Carbon::today()->copy()->addDay()->format('Y-m-d');
    }
       $guests = 2; 
       $rooms = 1;
      ...
      if ($response->successful()) {
          // Parse JSON safely
          $data = $response->json();
          if (!empty($data) && isset($data['searchId'])) {
              $searchId = $data['searchId'];    
          ...
          $jsonResponse = $response2->json();
           ...
           if (is_array($jsonResponse) && ($jsonResponse['status'] ?? null) === 'ok') {
            // proceed processing
...
if (!$getdata->isEmpty()) {
    ...
    foreach ($rooms as $room) {
        ...
        DB::table('TPRoomtype_tmp')->updateOrInsert(
            ...
            [
                ...
                'beds' => isset($options['beds']['double']) ? $options['beds']['double'] : 0,
                'doublebed' => isset($options['beds']['double']) ? $options['beds']['double'] : 0,
                'twin' => isset($options['beds']['twin']) ? $options['beds']['twin'] : 0,
                'balcony' => $options['balcony'] ?? 0,
                'privateBathroom' => $options['privateBathroom'] ?? 0,
            	'images' => isset($options['images']) ? (is_array($options['images']) ? json_encode($options['images']) : $options['images']) : '',
    ...
    return response()->json(['status' => 'ok', 'message' => 'Data updated in TPRoomtype_tmp and TPRoomtype']);
} else {
    $roomData = [];
    foreach ($rooms as $room) {
        $desc = $room['desc'];
    ...
    return "Data inserted into TPRoomtype_tmp and TPRoomtype";
           ...
           // If we reach here, the external API didn't return usable data
           return response()->json(['status' => 'no_data'], 200);
      }
      // External API call failed
      return response()->json(['status' => 'upstream_error', 'code' => $response->status()], 200);
   } catch (\Throwable $e) {
      logger()->error('insert_hotel_desction error: '.$e->getMessage(), [
        'trace' => $e->getTraceAsString()
      ]);
      return response()->json(['status' => 'error', 'message' => 'Temporary server issue'], 500);
   }
 }
				...
				if ($response->successful()) {
					...
					if(!empty($data)){
						...
						if ($response2->successful()) {                 
							...
							try { DB::statement('SET SESSION group_concat_max_len = 1000000'); } catch (\Throwable $e) {}
							...
							$searchresults = DB::table('TPHotel as h')
								->select('h.hotelid', 'h.id', 'h.name', 'h.slug', 'h.stars', 'h.rating', 
									...
									DB::raw("GROUP_CONCAT(DISTINCT CONCAT(REPLACE(a.shortName, ',', ' '), '|', COALESCE(a.image, '')) ORDER BY a.name SEPARATOR '|||') as amenity_info"))
								->leftJoin('TPHotel_amenities as a', DB::raw('FIND_IN_SET(a.id, h.shortFacilities)'), '>', DB::raw('0'))
								->whereIn('h.hotelid', $idArray)
								->whereNotNull('h.slugid')
							...
							if (!empty($hotel->result)) {
								...
								// debug log removed: inserting data
							...
							// End code
							$hotelIds = array_unique($searchresults->pluck('hotelid')->toArray());                       $hotelpricedata = DB::table('hotelbookingstemp')
...
public function addLocationfaqfont(request $request){
   ...
   $locationid = $request->get('locationIdValue');
    // Guard: required location id
    if (empty($locationid)) {
        return response()->json(['status' => 'error', 'message' => 'Missing required parameter: locationIdValue'], 400);
    }

   
        $getloc = DB::table('Location')->where('LocationId', $locationid)->first();
      if (!$getloc) {
          return response()->json(['status' => 'error', 'message' => 'Invalid locationIdValue'], 404);
      }
      $lname = $getloc->Name;
...
if (!$existingEntry_outdoor) {
        if(!empty($outkids_site)){
            // Ensure data fits in the column (assuming VARCHAR(255))
            // Start with just 1 activity and add more if there's space
            $limitedActivities = [];
            if (!empty($outkids_site)) {
                $limitedActivities[] = $outkids_site[0];
                
                // Try to encode with just one item first
                $jsonString = json_encode($limitedActivities);
                
                // If we have space for more and there are more available, add them
                if (strlen($jsonString) < 200 && count($outkids_site) > 1) {
                    $limitedActivities[] = $outkids_site[1];
                    $jsonString = json_encode($limitedActivities);
                    
                    // Try to add a third if there's still space
                    if (strlen($jsonString) < 200 && count($outkids_site) > 2) {
                        $limitedActivities[] = $outkids_site[2];
                    }
                }
            }
            
            $firstKids = $getkidsoutdoor->first();
            $outdoorkds = array(
                'LocationId'=>$locationid,
                'slugid'=> $firstKids ? $firstKids->slugid : null,
                'Question'=>'What the best outdoor activities for Kids in '. $lname,
                'Answer' => 'The best outdoor activities for Kids in ' . $lname.' are:',
                'listing'=>json_encode($limitedActivities),
                'CreatedDate' => now(),
            );
            ...
            try {
                ...
                // If still too large, try with just one item
                if (count($limitedActivities) > 1) {
                    $outdoorkds['listing'] = json_encode([$limitedActivities[0]]);
              DB::table('LocationQuestion')->insert($outdoorkds);
                }
            }
        }
  ...
  if (!$existingEntry) {
      ...
      $data = array(
          'LocationId'=>$locationid,
          'slugid'=> (isset($gettopatt[0]) ? $gettopatt[0]->slugid : null),
  ...
  if (!$existingEntry_outdoor) {
          if(!empty($outdoorloc)){
              $outdoor = array(
                 'LocationId'=>$locationid,
                 'slugid'=> (isset($getoutdoor[0]) ? $getoutdoor[0]->slugid : null),
                 'Question'=>'What are the best outdoor activities in '. $lname,
                 'Answer' => 'The best outdoor activities in ' . $lname.' are:',
                 'listing'=>json_encode($outdoorloc),
                 'CreatedDate' => now(),
      ...
      if (!$alredyexist) {
              if(!empty($child_loc)){
                  $childlocdata = array(
                     'LocationId'=>$locationid,
                     'slugid'=> (isset($getsights_child[0]) ? $getsights_child[0]->slugid : null),
...
//start sight faq
     ...
     //    aad reviews
        ...
        public function add_sightreview(Request $request){
        	...
        	$request->merge([
                'SightId' => $request->input('SightId', $request->input('sightId'))
            ]);
            
            $validated = $request->validate([
                'review' => 'required|string',
                'rating' => 'required|integer|min:1|max:5',
                'SightId' => 'required|integer|exists:Sight,SightId',
                'gowith' => 'nullable|string|max:255',
            ]);

            $userId = null;
            // Check if user is authenticated via session
            if (session()->has('frontend_user') && isset(session('frontend_user')['UserId'])) {
                $userId = session('frontend_user')['UserId'];
                $getuser = DB::table('FrontendUserLogin')->where('UserId', $userId)->first();
                if (!$getuser) {
                    return response()->json(['error' => 'User not found'], 404);
                }
            } else {
                // For guests, check if user ID is provided in request
                $userId = $request->get('userId');
                if (empty($userId) || $userId === '0') {
                    return response()->json(['error' => 'Authentication required to submit a review'], 401);
                }
            ...
            $data = [
                'CreatedDate' => now(),
                'ReviewDescription' => $validated['review'],
                'ReviewRating' => $validated['rating'],
                'SightId' => $validated['SightId'],
                'UserId' => $userId,
                'gowith' => $validated['gowith'] ?? null,
                'IsRecommend' => $validated['rating'] >= 3 ? 1 : 0,
            ];

            $result = DB::table('SightReviews')->insertGetId($data);
            
            if (!$result) {
                return response()->json(['error' => 'Failed to insert review'], 500);
            }
            ...
            if (!empty($uploadedFiles)) {
                foreach ($uploadedFiles as $image) {
                    if ($image->isValid()) {
                        ...
                        DB::table('sight_review_image')->insert($imageData);
                    }
                }
            }
                if($result){         
                    $getrv = DB::table('SightReviews')
                  ->where('SightReviews.SightId', $validated['SightId'])
                  ->leftJoin('FrontendUserLogin', 'FrontendUserLogin.UserId', '=', 'SightReviews.UserId')
                  ->select([
                      'FrontendUserLogin.FirstName',
                      'SightReviews.IsRecommend',
 ...
 // add reviews end
  ...
  //restaurant
        ...
        private function parseRestaurantSearchQuery(string $query): array
        {
            $q = trim($query);
            if ($q === '') {
                return [
            ...
            if (preg_match('/\s+near\s+(.+)$/i', $q, $m)) {
                $location = trim($m[1]);
                $term = trim(preg_replace('/\s+near\s+(.+)$/i', '', $q));
            }
        ...
        private function buildRestaurantNoResultsSuggestions(string $term, ?string $locationName): array
        {
            $term = trim($term);
            $locationName = $locationName !== null ? trim($locationName) : null;

            $s1 = $term !== '' ? ($term . ' near me') : 'Restaurants near me';
            if ($term !== '' && $locationName) {
                $s2 = $term . ' near ' . $locationName;
            } elseif ($term !== '') {
                $s2 = $term . ' near London';
            } else {
                $s2 = 'Restaurants near London';
            }
        ...
        public function searchlist(Request $request)
        {
            ...
            $slugid = $request->query('slugid');

            $parsed = $this->parseRestaurantSearchQuery($query);
            $term = $parsed['term'] ?? '';
            $locationFromQuery = $parsed['location'] ?? null;
            ...
            try {
                if (!empty($slugid)) {
                    $locationName = DB::table('Location')->where('slugid', $slugid)->value('Name');
                    $locationSlugid = $slugid;
                    $locationId = DB::table('Location')->where('slugid', $slugid)->value('LocationId');
                } elseif (!empty($locId)) {
                    $locationName = DB::table('Location')->where('LocationId', $locId)->value('Name');
                    $locationId = $locId;
                    $locationSlugid = DB::table('Location')->where('LocationId', $locId)->value('slugid');
                }
            } catch (\Throwable $e) {
                $locationName = null;
                $locationSlugid = null;
            ...
            if (!$locationName && $locationFromQuery && strtolower($locationFromQuery) !== 'me') {
                $locationName = $locationFromQuery;
            }
            ...
            if ($term !== '') {
                $searchExecuted = true;
                try {
                    if (!empty($locationSlugid)) {
                        $like = '%' . str_replace(['\\', '%', '_'], ['\\\\', '\\%', '\\_'], $term) . '%';
                        $restaurants = DB::table('Restaurant as r')
                            ->where('r.slugid', $locationSlugid)
                            ->where(function ($q) use ($like) {
                                $q->where('r.Title', 'LIKE', $like)
                                  ->orWhere('r.About', 'LIKE', $like);
                            })
                            ->select('r.RestaurantId', 'r.Title', 'r.slugid', 'r.slug', 'r.Address', 'r.Cost', 'r.Cuisines', 'r.Img1')
                            ->limit(50)
                            ->get();
            ...
            $suggestions = [];
            if ($searchExecuted && ($restaurants instanceof \Illuminate\Support\Collection ? $restaurants->isEmpty() : empty($restaurants))) {
                $suggestions = $this->buildRestaurantNoResultsSuggestions($term, $locationName);
            }
            ...
            return view('searchlist', [
                ...
                'location_slugid' => $locationSlugid,
	...
	public function hotel_homepage(){
      ...
      public function list_hotelsloc(Request $request)
     {
         ...
         return Cache::remember($cacheKey, $cacheDuration, function() use ($searchText, $city) {
             ...
             $exactLocationParams = [
                 'limit' => 5,
                 'attributesToRetrieve' => ['id', 'name', 'slugid', 'slug', 'parentName', 'stateName', 'countryName']
             ];
             ...
             if (!$locationMatches->isEmpty()) {
                 ...
                 $transformedLocations = $locationMatches->map(function($item) {
                     // Build location hierarchy with proper formatting
                     $locationName = $item['name'] ?? '';
                     
                     // Try parentName first, then stateName as fallback
                     $parentName = '';
                     if (!empty($item['parentName'])) {
                         $parentName = $item['parentName'];
                     } elseif (!empty($item['stateName'])) {
                         $parentName = $item['stateName'];
                     }
                     
                     $countryName = $item['countryName'] ?? '';
                     
                     // Create display name with all parts of the hierarchy
             ...
             if (!$foundExactLocation) {
                 ...
                 // Meilisearch filters do not support LIKE. Keep original idea commented for reference.
                 // if ($city) {
                 //     $hotelParams['filter'] = 'cityName LIKE "' . addslashes($city) . '%"';
                 // }
                 $hotelSearchTerm = $searchText;
                 if ($city) {
                     $hotelSearchTerm = trim($searchText . ' ' . $city);
                 }
                 
                 $hotelResults = $hotelsIndex->search($hotelSearchTerm, $hotelParams);
                 ...
                 if (!$hotelMatches->isEmpty()) {
                     ...
                     $transformedHotels = $hotelMatches->map(function($item) {
                         // Build hotel location hierarchy with proper formatting
                         $hotelName = $item['name'] ?? '';
            ...
            $result = $sortedResults->map(function($item) {
                // Make sure displayName includes all location hierarchy parts
                $displayName = $item->displayName;
                
                // Split the displayName to separate name and parent/country info
                $nameParts = explode(',', $displayName, 2);
                $mainName = trim($nameParts[0]);
                $parentInfo = isset($nameParts[1]) ? trim($nameParts[1]) : '';
                
                $base = [
                    'id' => $item->id,
                    'value' => $displayName, // Full location hierarchy for value field
                    'name' => $mainName,     // Just the location name
                    'parent_info' => $parentInfo, // Parent and country info
                    'Slug' => $item->Slug,
                    'hotel' => $item->is_hotel
                ];
...
public function recenthotels(Request $request)
{
    ...
    // Check both session keys for search history
    $sessionKeys = ['lasthotelsearch', 'lastsearch', 'recent_sightlist_searches', 'recent_hotellist_searches'];
    ...
    foreach ($sessionKeys as $key) {
        ...
        try {
            ...
            if (is_array($data)) {
                // If it's an indexed array of searches
                if (array_values($data) === $data) {
                    $allSearches = array_merge($allSearches, $data);
                } else {
                    // It's a single search item
                    $allSearches[] = $data;
                }
            }
        } catch (\Exception $e) {
            \Log::error('Error processing search history: ' . $e->getMessage());
            continue;
        }
    }
    ...
    if (!empty($allSearches)) {
        ...
        // Process in reverse to get most recent first
        $reversedSearches = array_reverse($allSearches);
        
        foreach ($reversedSearches as $value) {
            if (!is_array($value)) {
                continue;
            }
            
            // Handle different data structures
            $id = $value['id'] ?? $value['locationId'] ?? $value['hotelid'] ?? null;
            $name = $value['Name'] ?? $value['val'] ?? $value['displayname'] ?? $value['name'] ?? null;
            $slug = $value['key'] ?? $value['slug'] ?? '';
            ...
            $item = [
                ...
                'value' => $name,
                'hotel' => $value['hotel'] ?? 0,
                'hotelid' => $value['hotelid'] ?? $id
            ];
            
            // Handle different search types
            if (isset($value['type'])) {
                $item['type'] = $value['type'];
                if ($value['type'] === 'sight' && isset($value['SightId'])) {
                    $item['SightId'] = $value['SightId'];
                }
            }
            
            // Add additional fields if they exist
            $additionalFields = ['SightId', 'locationId', 'categoryId', 'type'];
	...
	//weather
      ...
      //filter hotel list
		...
		->distinct()
           ...
           if (!empty($amenities)) {
                $searchresults->whereHas('amenities', function ($query) use ($amenities) {
                    $query->whereIn('name', $amenities);
                });
            }
            //new code
            if (!empty($agency)) {
...
public function storeHighlightReview(Request $request)
{
    ...
    {
        if ($request->has('val')) {
            $searchText = trim((string) $request->input('val'));
            $locId = $request->input('locId');
            $searchDebug = (bool) env('SEARCH_DEBUG_SIGHTLISTING', false);

            if ($searchText === '') {
                return response()->json([]);
            }

            $buildNoResultsSuggestions = function($term, $locationName = null) {
                $term = trim((string) $term);
                $locationName = trim((string) ($locationName ?? ''));

                $inText = $term;
                if ($locationName !== '') {
                    $inText .= ' in ' . $locationName;
                } else {
                    $inText .= ' in this area';
                }
            ...
            // Fallback: if locId not provided, try mapping from slugid
            if (empty($locId) && $request->has('slugid')) {
                $slugid = $request->input('slugid');
                try {
                    $mapped = Cache::remember('searchsightlisting_locid_by_slugid_' . $slugid, 3600, function () use ($slugid) {
                        return DB::table('Location')->where('slugid', $slugid)->value('LocationId');
                    });
                    if (!empty($mapped)) {
                        $locId = $mapped;
                        if ($searchDebug) {
                            Log::info('searchsightlisting locId mapped from slugid', ['slugid' => $slugid, 'locId' => $locId]);
                        }
                    } else {
                        if ($searchDebug) {
                            Log::warning('searchsightlisting locId mapping failed', ['slugid' => $slugid, 'locId' => $locId]);
                        }
                    }
                } catch (\Throwable $e) {
                    if ($searchDebug) {
                        Log::error('searchsightlisting locId mapping exception', ['slugid' => $slugid, 'message' => $e->getMessage()]);
                    }
                }
            }

            // Fallback: legacy callers may not send locId/slugid; map from Referer URL (/lo-<slugid>-...)
            if (empty($locId)) {
                try {
                    $referer = (string) $request->headers->get('referer');
                    if (!empty($referer) && preg_match('/\/lo-(\d+)/i', $referer, $m)) {
                        $slugidFromRef = $m[1];
                        $mappedFromRef = Cache::remember('searchsightlisting_locid_by_slugid_' . $slugidFromRef, 3600, function () use ($slugidFromRef) {
                            return DB::table('Location')->where('slugid', $slugidFromRef)->value('LocationId');
                        });
                        if (!empty($mappedFromRef)) {
                            $locId = $mappedFromRef;
                            if ($searchDebug) {
                                Log::info('searchsightlisting locId mapped from referer', ['slugid' => $slugidFromRef, 'locId' => $locId]);
                            }
                        } else {
                            if ($searchDebug) {
                                Log::warning('searchsightlisting referer mapping failed', ['slugid' => $slugidFromRef, 'referer' => $referer]);
                            }
                        }
                    }
                } catch (\Throwable $e) {
                    if ($searchDebug) {
                        Log::error('searchsightlisting referer mapping exception', ['message' => $e->getMessage()]);
                    }
                }
            }
            ...
            try {
                if ($searchDebug) {
                    Log::info('searchsightlisting request', ['val' => $searchText, 'locId' => $locId]);
                }
            } catch (\Throwable $e) {
            ...
            if (empty($locId)) {
                try { if ($searchDebug) { Log::warning('searchsightlisting missing locId after fallback', ['val' => $searchText]); } } catch (\Throwable $e) {}
                $locationName = null;
                try {
                    $slugid = $request->input('slugid');
                    if (!empty($slugid)) {
                        $locationName = Cache::remember('searchsightlisting_location_name_by_slugid_' . $slugid, 3600, function () use ($slugid) {
                            return DB::table('Location')->where('slugid', $slugid)->value('Name');
                        });
                    }
                } catch (\Throwable $e) {
                    $locationName = null;
                ...
                return response()->json($buildNoResultsSuggestions($searchText, $locationName));
            }

            $cacheKey = 'searchsightlisting_v2:' . $locId . ':' . md5(strtolower($searchText));
            $cacheTtlSeconds = 30;

            return response()->json(Cache::remember($cacheKey, $cacheTtlSeconds, function () use ($locId, $searchText, $buildNoResultsSuggestions, $searchDebug) {
            try {
            $response = [];
            $maxTotal = 18;
            ...
            $locationSlugid = null;
            try {
                $locationSlugid = Cache::remember('searchsightlisting_slugid_by_locid_' . $locId, 3600, function () use ($locId) {
                    return DB::table('Location')->where('LocationId', $locId)->value('slugid');
                });
            } catch (\Throwable $e) {
                $locationSlugid = null;
            }

            $prefixLike = str_replace(['\\', '%', '_'], ['\\\\', '\\%', '\\_'], $searchText) . '%';
            $containsLike = '%' . str_replace(['\\', '%', '_'], ['\\\\', '\\%', '\\_'], $searchText) . '%';

            // Must See category shortcut (keep behavior)
            if (strtolower(trim($searchText)) === 'must see') {
                $response[] = [
                    'id' => '1',
                    'value' => 'Must See',
                    'type' => 'Category'
            ...
            $sights = DB::table('Sight')
                ->where('Sight.LocationId', $locId)
                ->where(function ($query) use ($prefixLike) {
                    $query->where('Sight.Title', 'LIKE', $prefixLike);
                })
                ->select('Sight.SightId as id', 'Sight.Title as displayname', 'Sight.Slug as slug')
                ->limit(5)
                ->get();
            try { if ($searchDebug) { Log::info('searchsightlisting sights', ['count' => $sights->count(), 'locId' => $locId, 'val' => $searchText]); } } catch (\Throwable $e) {}
            foreach ($sights as $row) {
                $response[] = [
                    'id' => $row->id,
                    'value' => $row->displayname,
                    'type' => 'Sight',
                    'slugid' => $locationSlugid,
                    'slug' => $row->slug ?? null
                ];
            }

            if (count($response) >= $maxTotal) {
                return array_slice($response, 0, $maxTotal);
            }
            ...
            $experiences = DB::table('Experience as e')
                ->where('e.LocationId', $locId)
                ->where(function ($query) use ($prefixLike) {
                    $query->where('e.Name', 'LIKE', $prefixLike);
                })
                ->select('e.ExperienceId as id', 'e.Name as displayname', 'e.Slug as slug')
                ->limit(5)
                ->get();
            try { if ($searchDebug) { Log::info('searchsightlisting experiences', ['count' => $experiences->count(), 'locId' => $locId, 'val' => $searchText]); } } catch (\Throwable $e) {}
            ...
            // Categories (based on Sight->Category)
            $categories = DB::table('Sight')
                ->leftJoin('Category', 'Sight.categoryId', '=', 'Category.categoryId')
                ->where('Sight.LocationId', $locId)
                ->where(function ($query) use ($prefixLike) {
                    $query->where('Category.Title', 'LIKE', $prefixLike);
                })
                ->distinct()
                ->select('Category.categoryId as id', 'Category.Title as displayname')
                ->limit(5)
                ->get();
            try { if ($searchDebug) { Log::info('searchsightlisting categories', ['count' => $categories->count(), 'locId' => $locId, 'val' => $searchText]); } } catch (\Throwable $e) {}
            ...
            if (!empty($locationSlugid)) {
                // Restaurants (Title)
                $restaurantsByTitle = DB::table('Restaurant as r')
                    ->where('r.slugid', $locationSlugid)
                    ->where(function ($query) use ($prefixLike) {
                        $query->where('r.Title', 'LIKE', $prefixLike);
                    })
                    ->distinct()
                    ->select('r.RestaurantId as id', 'r.Title as displayname', 'r.slugid as loc_slugid', 'r.slug as slug')
                    ->limit(5)
                    ->get();
                try { if ($searchDebug) { Log::info('searchsightlisting restaurants_title', ['count' => $restaurantsByTitle->count(), 'locId' => $locId, 'val' => $searchText]); } } catch (\Throwable $e) {}
                foreach ($restaurantsByTitle as $row) {
                    $response[] = [
                        'id' => $row->id,
                        'value' => $row->displayname,
                        'type' => 'Restaurant',
                        'slugid' => $row->loc_slugid ?? null,
                        'slug' => $row->slug ?? null
                    ];
                }
            }
            ...
            if (!empty($locationSlugid)) {
                // Restaurants (About)
                $restaurantsByAbout = DB::table('Restaurant as r')
                    ->where('r.slugid', $locationSlugid)
                    ->where(function ($query) use ($containsLike) {
                        $query->where('r.About', 'LIKE', $containsLike);
                    })
                    ->select('r.RestaurantId as id', 'r.Title as displayname', 'r.slugid as loc_slugid', 'r.slug as slug')
                    ->limit(5)
                    ->get();
                try { if ($searchDebug) { Log::info('searchsightlisting restaurants_about', ['count' => $restaurantsByAbout->count(), 'locId' => $locId, 'val' => $searchText]); } } catch (\Throwable $e) {}
                foreach ($restaurantsByAbout as $row) {
                    $response[] = [
                        'id' => $row->id,
                        'value' => $row->displayname,
                        'type' => 'Restaurant',
            ...
            if (!empty($locationSlugid)) {
                // Restaurants (Cuisine)
                $restaurantsByCuisine = DB::table('Restaurant as r')
                    ->join('RestaurantCuisineAssociation as ra', 'ra.RestaurantId', '=', 'r.RestaurantId')
                    ->join('RestaurantCuisine as rc', 'ra.RestaurantCuisineId', '=', 'rc.RestaurantCuisineId')
                    ->where('r.slugid', $locationSlugid)
                    ->where(function ($query) use ($containsLike) {
                        $query->where('rc.Name', 'LIKE', $containsLike);
                    })
                    ->select('r.RestaurantId as id', 'r.Title as displayname', 'r.slugid as loc_slugid', 'r.slug as slug')
                    ->limit(5)
                    ->get();
                try { if ($searchDebug) { Log::info('searchsightlisting restaurants_cuisine', ['count' => $restaurantsByCuisine->count(), 'locId' => $locId, 'val' => $searchText]); } } catch (\Throwable $e) {}
                foreach ($restaurantsByCuisine as $row) {
                    $response[] = [
                        'id' => $row->id,
                        'value' => $row->displayname,
                        'type' => 'Restaurant',
            ...
            // De-duplicate by (type,id) while preserving order
            $seen = [];
            $deduped = [];
            ...
            if (empty($deduped)) {
                $locationName = null;
                try {
                    $locationName = Cache::remember('searchsightlisting_location_name_by_locid_' . $locId, 3600, function () use ($locId) {
                        return DB::table('Location')->where('LocationId', $locId)->value('Name');
                    });
                } catch (\Throwable $e) {
                    $locationName = null;
                }
                $deduped = $buildNoResultsSuggestions($searchText, $locationName);
            }

            try { if ($searchDebug) { Log::info('searchsightlisting final', ['count' => count($deduped), 'locId' => $locId, 'val' => $searchText]); } } catch (\Throwable $e) {}
            return $deduped;
            } catch (\Throwable $e) {
                if ($searchDebug) {
                    Log::error('searchsightlisting exception', ['message' => $e->getMessage(), 'locId' => $locId, 'val' => $searchText]);
                }
                $locationName = null;
                try {
                    $locationName = Cache::remember('searchsightlisting_location_name_by_locid_' . $locId, 3600, function () use ($locId) {
                ...
                return $buildNoResultsSuggestions($searchText, $locationName);
            }
            }));
        }
    }
