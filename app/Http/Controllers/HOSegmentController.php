...
class HOSegmentController extends Controller
{
    ...
    public function hotel_list($segment, Request $request)
    {
        ...
        Log::debug('HOTEL_LIST - Request received', [
            'segment' => $segment,
        ...
        if (!empty($parts)) {
            ...
            foreach ($parts as $part) {
                ...
                if (isset($processed_amenities[$part]) || isset($processed_neighborhoods[$part]) || isset($processed_sights[$part])) {
                    continue;
                }
        ...
        switch ($part) {
            ...
            case 'breakfast':
                ...
                elseif (preg_match('/^pt(\d+)$/', $part, $matches)) {
                    ...
                    // Regardless of the type name, only keep the ID token in URL (no slug)
                    $filter_parts[] = $part;
                    // Mark processed
                    $processed_propertyTypes[$part] = true;
                    // Store just the part (ptX) for later use
                ...
                elseif (preg_match('/^a[a-z]+(\d+)$/', $part, $matches)) {
                    ...
                    // Only keep the ID token in URL (no appended name)
                    $filter_parts[] = $part;
    ...
    if ($location_info && $neighborhood_info->LocationID == $location_info->LocationId) {
        ...
        abort(404); // Show a 404 error if the neighborhood does not belong to the location
                ...
                // If it's a text part following an amenity or neighborhood ID, skip it
                elseif (preg_match('/^[a-zA-Z\-]+$/', $part) && 
                       ($current_part_type == 'amenity' || $current_part_type == 'neighborhood' || $current_part_type == 'sight')) {
                    continue;
                }
        ...
        // Log before checking Temp_Mapping
        Log::debug('HOTEL_LIST - Checking location in Temp_Mapping', [
            'slugid' => $id,
            'current_slug' => $slug ?? 'not_set'
        ]);
        
        // Check if location exists in Temp_Mapping by slugid
        $locationExists = DB::table('Temp_Mapping')->where('slugid', $id)->first();
        
        // Log Temp_Mapping check result
        Log::debug('HOTEL_LIST - Temp_Mapping check result', [
            'exists' => $locationExists ? 'yes' : 'no',
            'found_slug' => $locationExists ? ($locationExists->slug ?? 'no_slug') : 'n/a'
        ]);
    ...
    if (strpos($currentUrl, $baseCorrectUrl) === false) {
        ...
        foreach ($allParts as $part) {
            // Keep only ID tokens or special text filters; ignore descriptive names
            if (preg_match('/^(st\d+|rs\d+|price\d+|pt\d+|a[a-z]+\d+|n[a-z]+\d+|sqx\d+)/', $part) ||
                ...
                // Otherwise it's part of the location slug or legacy text we ignore for filters
        ...
        // Log the redirect
        Log::debug('HOTEL_LIST - Redirecting to correct URL', [
            'from' => 'ho-' . $segment,
            'to' => $correctUrl,
            'reason' => 'Location found with different slug'
        ]);
       ...
       $metadata =collect();
        $filterPattern = '/(fl[a-z]+[0-9a-zA-Z_]+)/';
            ...
            if ($locationInfo) {
                ...
                $lslug = $locationInfo->Slug; // Use the slug from Location table
                ...
                // Update the slug variable to match the one from Location table
                $slug = $lslug;
            ...
            if( $chkin !="" && $checout !=""){
              ...
              //  $getloc = DB::table('TPLocations')->select('fullName','cityName','countryName')->where('id',$locationid)->first();
                        ...
                        //new code  
                          ...
                          if ($status == 1 && $response2->successful()) {
                              ...
                              $searchresults = Cache::remember($cacheKey, 300, function() use ($idArray) {
                                  // Check which facilities column exists
                                  $hasShortFacilities = Schema::hasColumn('TPHotel', 'shortFacilities');
                                  $hasFacilitiesUnderscore = Schema::hasColumn('TPHotel', 'facilities_');
                                  $hasFacilities = Schema::hasColumn('TPHotel', 'facilities');

                                  $facilitiesColumn = $hasShortFacilities ? 'h.shortFacilities' : ($hasFacilitiesUnderscore ? 'h.facilities_' : 'h.facilities');
                                  ...
                                  $query = DB::table('TPHotel as h')
                                      ...
                                      ->leftJoin('TPHotel_amenities as a', function($join) use ($facilitiesColumn) {
                                          $join->whereRaw('FIND_IN_SET(a.id, ' . $facilitiesColumn . ') > 0');
                                      })
                              ...
                              // Deep log: sample hotels' facilities and amenity_info before mapping
                              $logSample = [];
                              foreach ($searchresults->take(10) as $h) {
                                  $logSample[] = [
                                      'hotelid' => $h->hotelid ?? null,
                                      'name' => $h->name ?? null,
                                      'facilities' => $h->facilities ?? null,
                                      'facilities_' => $h->facilities_ ?? null,
                                      'shortFacilities' => $h->shortFacilities ?? null,
                                      'room_aminities' => $h->room_aminities ?? null,
                                      'amenity_info_raw' => $h->amenity_info ?? null,
                                  ];
                              }

                              // Transform the results if needed
                              $searchresults = $searchresults->map(function($hotel) {
                                  $hotel->amenity_info = !empty($hotel->amenity_info) ? $hotel->amenity_info : '';
                                  return $hotel;
                              });
                              ...
                              foreach ($searchresults->take(10) as $h) {
                                  $logSampleAfter[] = [
                                      'hotelid' => $h->hotelid ?? null,
                                      'amenity_info' => $h->amenity_info ?? null,
                                  ];
                              }                    
                ...
                // First, get the location from Temp_Mapping using slugid only
                $getloc = DB::table('Temp_Mapping as tm')  
                    ->select('tm.LocationId', 'tm.cityName', 'tm.countryName', 'tm.slug as actual_slug')
                    ->where('tm.slugid', $desiredId)
                    ->limit(1)
                    ->get();
                    
                // If not found, try with the slug from Location table
                if ($getloc->isEmpty() && isset($locationInfo)) {
                    $getloc = DB::table('Temp_Mapping as tm')
                        ->select('tm.LocationId', 'tm.cityName', 'tm.countryName', 'tm.slug as actual_slug')
                        ->where('tm.slug', $locationInfo->Slug)
                        ->limit(1)
                        ->get();
                }
                    
                Log::debug('HOTEL_LIST - Location lookup result', [
                    'slugid' => $desiredId,
                    'tried_slugs' => [
                        'original' => $slug,
                        'from_location_table' => $locationInfo->Slug ?? 'n/a'
                    ],
                    'found' => $getloc->isNotEmpty() ? 'yes' : 'no',
                    'found_slug' => $getloc->isNotEmpty() ? ($getloc[0]->actual_slug ?? 'unknown') : 'none',
                    'query' => DB::table('Temp_Mapping')->where('slugid', $desiredId)->toSql()
                ]);
      
                if($getloc->isEmpty()){
                    // Log the 404 error with more context
                    Log::debug('HOTEL_LIST - 404 error: Location not found in Temp_Mapping', [
                        'slugid' => $desiredId,
                        'locationid' => $desiredId,
                        'slug_used' => $slug,
                        'normalized_slug' => str_replace('--', '-', $slug),
                        'query' => DB::table('Temp_Mapping')
             ...
             // Initialize results as paginator using unified filtering logic (no caching on paginator)
               $searchresults = collect();
               ...
               try {
                   ...
                   // Special amenity-based room joins
                   if (!empty($amenity) && ($amenity === 'free cancellation' || $amenity === 'breakfast')) {
                   ...
                   // Amenity IDs (FIND_IN_SET like DataController alt path)
                   ...
                   // Sights proximity filter (3km) if available in this method's context
                   if (!empty($sights)) {
                       $query->where(function($q1) use ($sights) {
                           foreach ($sights as $sight) {
                               $q1->orWhere(function($q) use ($sight) {
                                   $q->whereRaw('
                                       ROUND(
                                           111.045 * SQRT(
                   ...
                   // Neighborhood proximity filter (2km)
                   if (!empty($neighborhoods)) {
                       $query->where(function($q1) use ($neighborhoods) {
                           foreach ($neighborhoods as $neighborhood) {
                               $q1->orWhere(function($q) use ($neighborhood) {
                                   $q->whereRaw('
                                       ROUND(
                                           111.045 * SQRT(
                                               POWER(ABS(CAST(h.Latitude AS DECIMAL(10,6)) - ?), 2) +
                   ...
                   // Text amenity filters similar to DataController alt path
                   if (!empty($amenity)) {
                       if ($amenity === 'parking' || $amenity === 'wifi' || $amenity === 'Internet') {
                           $query->whereExists(function($sub) use ($amenity) {
                               $sub->select(DB::raw(1))
                                   ->from('TPHotel_amenities as a2')
                                   ->whereRaw('FIND_IN_SET(a2.id, h.facilities)')
                                   ->where(function($qq) use ($amenity) {
                                       $qq->where('a2.name', 'LIKE', "%{$amenity}%")
                   ...
                   $count_result = $searchresults->count();
                       ...
                       // Log the error
                       Log::debug('HOTEL_LIST - Error in search query', [
                           'error' => $e->getMessage(),
                           'query' => $query->toSql(),
                           'bindings' => $query->getBindings()
                       ]);
                       // swallow and continue with empty results
               }
    ...
    if ($sight_slugs) {
        ...
        foreach ($sight_parts as $part) {
           ...
           $sight_info = empty($sight_ids) ? collect(): DB::table('Sight')
        	->select(
            'SightId',
            'Title',
            'About',
	...
	$location_info = DB::table('Location')
    ->select('heading', 'headingcontent' , 'Name', 'about_hotel')
    ->where('slugid', $id)
    ->first();
    ...
    return view('hotel_listing')
        ...
        private function getHotelPropertyTypes($locationId, $locationSlug)
        {
            ...
            return Cache::remember($cacheKey, 86400, function() use ($locationId, $locationSlug) {
                ...
                $locationInDb = DB::table('Location')
                    ->where('slugid', $locationId)
                    ->first();
                    
                Log::debug('HOTEL_LIST - Location table check result', [
                    'found' => $locationInDb ? 'yes' : 'no',
                    'location_id' => $locationInDb->LocationId ?? 'not_found',
                    'slug' => $locationInDb->Slug ?? 'no_slug'
                ]);
