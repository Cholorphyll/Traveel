...
public function store_hotel_qa(Request $request){
    try {
        ...
        \Log::info('Creating new hotel question:', [
            'hotel_id' => $request->hotel_id,
            'Question' => $request->Question,
            'Answer' => substr($request->Answer, 0, 50) . '...', // Log only part of the answer
        ]);
        ...
        $questionId = DB::table('HotelQuestion')->insertGetId([
            ...
            'updatedOn' => DB::raw('NOW()')
        ]);
        
        \Log::info('New hotel question created:', ['questionId' => $questionId, 'hotelId' => $request->hotel_id]);
        
        return redirect()->back()->with('success', 'Question added successfully!');
    } catch (\Exception $e) {
        \Log::error('Error creating hotel question:', [
            'error' => $e->getMessage(),
        ...
        return redirect()->back()
            ->withInput()
            ->withErrors(['error' => 'Failed to add question: ' . $e->getMessage()]);
    }
}
// qa hotel
public function qa_hotel($id){
    // Get hotel information directly with simple query
    $gethid = DB::table('TPHotel')
        ->where('id', $id)
        ->get();
        
    // Fetch questions using the query builder, assuming HotelId is an integer
    $questions = DB::table('HotelQuestion')
        ->where('HotelId', (int)$id)
        ->get();
    
    return view('hotels.qa_hotel', ['questions' => $questions, 'gethid' => $gethid]);
}
...
public function update_hotel_qa(Request $request){
    try {
        \Log::info('Updating question:', ['hotelQuestionId' => $request->hotelQuestionId, 'Question' => $request->Question]);
        
        $validated = $request->validate([
            'hotelQuestionId' => 'required',
            'Question' => 'required|string|max:255',
            'Answer' => 'required|string'
        ]);
        
        // Update the question - try both field names
        $updated = DB::table('HotelQuestion')
            ->where('HotelId', $request->hotelQuestionId)
            ->update([
                'Question' => $request->Question,
                'Answer' => $request->Answer,
...
{
    $validated = $request->validate([
        'hotelQuestionId' => 'required|integer'
    ]);
    
    DB::table('HotelQuestion')
        ->where('hotelQuestionId', $request->hotelQuestionId)
        ->delete();
