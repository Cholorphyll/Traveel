...
@php
    ...
    $hasSights = isset($sight_info) && count($sight_info) > 0;
    $hasPropertyTypes = !empty($propertyType_ids) && isset($propertyType_info) && count($propertyType_info) > 0;
@endphp
@php
    $locationTitle = $lname;

    if ($hasNeighborhoods && isset($neighborhood_info) && count($neighborhood_info) > 0) {
        $neighborhoodNames = array_map(function ($n) {
            return $n->Name;
        }, $neighborhood_info->toArray());

        $locationTitle = implode(', ', $neighborhoodNames) . ($lname ? ', ' . $lname : '');
    }
@endphp
...
@php
    ...
    if ($hasSights && isset($sight_info) && count($sight_info) > 0) {
        $sightNames = implode(' and ', array_map(function ($s) {
            return $s->Title;
        }, $sight_info->toArray()));

        $title .= " near {$sightNames}";
    }

    $title .= " in {$locationTitle}";

    if ($hasPropertyTypes && !$hasNeighborhoods && !$hasAmenities && empty($amenity)) {
        $types = implode(', ', array_map(function ($pt) {
            return $pt->type;
        }, $propertyType_info->toArray()));

        $title = "{$types} {$title}";
    }

    if ($hasAmenities && isset($amenity_info) && count($amenity_info) > 0) {
        $amenities = implode(' and ', array_map(function ($a) {
            return $a->name;
    ...
    if (!empty($amenity)) {
        if ($amenity === "breakfast") $title .= " with Free Breakfast";
        elseif ($amenity === "parking") $title .= " with Free Parking";
        elseif ($amenity === "free_cancellation") $title .= " with Free Cancellation";
        elseif ($amenity === "Internet") $title .= " with Free Internet";
    }

    if (!empty($reviewscore)) {
        $title .= " with {$reviewscore}+ Review Score";
    }

    if (!empty($price)) {
        if (strpos($price, '-') !== false) {
            $priceRange = explode('-', $price);
            $formattedPrice = "₹{$priceRange[0]} - ₹{$priceRange[1]}";
        } else {
            $formattedPrice = "₹{$price}";
        }

        $title .= " under {$formattedPrice}";
    }
              ...
              <div class="tr-filter-lists">
                <h5>Search by price</h5>
                 <ul>
                  ...
                  <li class="tr-filter-list"><a href="{{ url('ho-'.$slgid .'-'.$slugdata.'-price50') }}">Under ₹500</a></li>
                  <li class="tr-filter-list"><a href="{{ url('ho-'.$slgid .'-'.$slugdata.'-price100') }}">Under ₹1000</a></li>
                  <li class="tr-filter-list"><a href="{{ url('ho-'.$slgid .'-'.$slugdata.'-price200') }}">Under ₹2000</a></li>
...
@endphp
        ...
        @php
            $locationTitle = $lname; // Default to city name
            if ($hasNeighborhoods && isset($neighborhood_info) && count($neighborhood_info) > 0) {
                $neighborhoodNames = array_map(function($neighborhood) { return $neighborhood->Name; }, $neighborhood_info->toArray());
                $locationTitle = implode(', ', $neighborhoodNames) . ($lname ? ', ' . $lname : '');
            }
            $titleParts = [];
            ...
            if ($hasPropertyTypes && !$hasNeighborhoods && !$hasSights && !$hasAmenities && $amenity == "") {
                $titleParts[] = implode(', ', array_map(function($propertyType) { return $propertyType->type; }, $propertyType_info->toArray()));
                $titleParts[] = "in " . $locationTitle;
            } else {
                $titleParts[] = "Hotels in " . $locationTitle;
            }

            if ($hasSights && count($sight_info) > 0) {
                $titleParts[] = "near " . implode(' and ', array_map(function($sight) { return $sight->Title; }, $sight_info->toArray()));
            }

            if ($hasAmenities) {
                $titleParts[] = "with " . implode(' and ', array_map(function($amenity) { return $amenity->name; }, $amenity_info->toArray()));
            }

            // Handle specific amenity cases if they are not covered by $hasAmenities
            if ($amenity != "") {
                if ($amenity == "breakfast") $titleParts[] = "Free Breakfast";
                elseif ($amenity == "parking") $titleParts[] = "Free Parking";
                elseif ($amenity == "free_cancellation") $titleParts[] = "Free cancellation";
                elseif ($amenity == "Internet") $titleParts[] = "Free internet";
            }

            if ($reviewscore != "") {
                $titleParts[] = "with a review score of " . $reviewscore;
            }

            echo "Showing " . implode(' ', $titleParts);
        @endphp
        ...
        if ($searchresult->amenity_info != "") {
            ...
            foreach ($amenityData as $item) {
                if (strpos($item, '|') !== false) {
                    list($rawName, $available) = explode('|', $item);
                    $rawName = trim($rawName);
                    $available = (int) trim($available);

                    // Normalize raw name to our supported set
                    $lower = strtolower($rawName);
                    $normalized = null;
                    if (preg_match('/laundry/', $lower)) { $normalized = 'Laundry'; }
                    elseif (preg_match('/wifi|wi-fi|wi fi|internet/', $lower)) { $normalized = 'Wi-Fi'; }
                    elseif (preg_match('/parking/', $lower)) { $normalized = 'Parking'; }
                    elseif (preg_match('/spa/', $lower)) { $normalized = 'Spa'; }
                    elseif (preg_match('/gym|fitness/', $lower)) { $normalized = 'Gym'; }
                    elseif (preg_match('/pool|swimming/', $lower)) { $normalized = 'Pool'; }
                    elseif (preg_match('/bar/', $lower)) { $normalized = 'Bar'; }
                    elseif (preg_match('/restaurant|food|dining/', $lower)) { $normalized = 'Food'; }
                    elseif (preg_match('/a\/c|air conditioning|air conditioner|climate/', $lower)) { $normalized = 'A/C'; }
                    elseif (preg_match('/smoke/', $lower)) { $normalized = 'Smoke-free'; }

                    if ($normalized && in_array($normalized, $selectedAmenities)) {
                        $amenities[] = [
                            'name' => $normalized,
                            'available' => $available,
                        ];
...
<div class="container">                                              
    @if(isset($sight_info) && $sight_info && count($sight_info) > 0)
            ...
            <div class="tr-single-page">
                <div class="tr-terms-and-conditions-section">
                    ...
                    <p style="margin-top: 20px;">
                        @if(isset($sight->About) && !empty($sight->About))
                            {!! $sight->About !!}
                        @else
                            <p>No detailed information available for this attraction.</p>
                        @endif
                    </p>
                </div>
            </div>
             ...
             <!--City Wise Hotel and Price - START-->
              ...
              <div class="tr-city-wise-hotel-listing">
                ...
                <div class="tr-city-wise-hotel-lists">
                        ...
                        @if(isset($amenity['name']) && isset($amenity['url']) && isset($amenity['code']))
                          <div class="tr-hotel-list">
                            <div class="tr-hotel-name">
                              <a href="{{ $amenity['url'] }}" title="{{ $lname }} Hotels with {{ $amenity['name'] }}" target="_blank" rel="noopener">{{ $lname }} Hotels with {{ $amenity['name'] }}</a>
                            </div>
                          </div>
                        @endif
                      @endforeach
                  ...
                  <div class="tr-city-wise-hotel-list">
                    <div class="tr-city-name">5 Stars Hotels in {{ $lname }}</div>
                    <div class="tr-hotel-lists">
                  ...
                  <div class="tr-city-wise-hotel-list">
                    <div class="tr-city-name">4 Stars Hotels in {{ $lname }}</div>
                    <div class="tr-hotel-lists">
                ...
                <div class="tr-city-wise-hotel-lists">
                    ...
                    <div class="tr-hotel-lists">
                      @foreach($hotelNeighborhoods as $neighborhood)
                        @if(isset($neighborhood['name']) && isset($neighborhood['url']) && isset($neighborhood['code']))
                          <div class="tr-hotel-list">
                            <div class="tr-hotel-name">
                              <a href="{{ $neighborhood['url'] }}" title="{{ $neighborhood['name'] }}" target="_blank" rel="noopener">{{ $neighborhood['name'] }}</a>
                            </div>
                          </div>
                        @endif
                      @endforeach
                  ...
                  <div class="tr-city-wise-hotel-list">
                    <div class="tr-city-name">3 Stars Hotels in {{ $lname }}</div>
                    <div class="tr-hotel-lists">
                  ...
                  <div class="tr-city-wise-hotel-list">
                    <div class="tr-city-name">2 Stars Hotels in {{ $lname }}</div>
                    <div class="tr-hotel-lists">
                ...
                <div class="tr-city-wise-hotel-lists">
                    ...
                    <div class="tr-hotel-lists">
                      @foreach($popularSections as $section)
                        @if(isset($section['name']) && isset($section['url']) && isset($section['code']))
                          <div class="tr-hotel-list">
                            <div class="tr-hotel-name">
                              <a href="{{ $section['url'] }}" title="{{ $section['name'] }}" target="_blank" rel="noopener">{{ $section['name'] }}</a>
                            </div>
                          </div>
                        @endif
                      @endforeach
                  ...
                  <div class="tr-city-wise-hotel-list">
                    <div class="tr-city-name">Budget Hotels in {{ $lname }}</div>
                    <div class="tr-hotel-lists">
                  ...
                  <div class="tr-city-wise-hotel-list">
                    <div class="tr-city-name">{{ $lname }} Hotels with Attractions Nearby</div>
                    <div class="tr-hotel-lists">
                      @foreach($nearbySights as $sight)
                        @if(isset($sight['name']) && isset($sight['url']) && isset($sight['code']))
                          <div class="tr-hotel-list">
                            <div class="tr-hotel-name">
                              <a href="{{ $sight['url'] }}" title="{{ $sight['name'] }}" target="_blank" rel="noopener">Hotels near {{ $sight['name'] }}</a>
                            </div>
                          </div>
                        @endif
                      @endforeach
...
@endif                                      
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="tr-single-page">
                <div class="tr-terms-and-conditions-section">
                    @if(isset($pricingStats['averageNightlyRate']) && ($pricingStats['averageNightlyRate']['min'] > 0 || $pricingStats['averageNightlyRate']['max'] > 0))
                    <h3 style="font-weight: bold; margin-bottom: 20px; font-size: 24px;">{{ $location_info->Name }} Hotel Pricing Guide</h3>
                    <p style="margin-top: 20px; margin-bottom: 20px;"> The average hotel cost in {{ $location_info->Name }} is ${{ $pricingStats['averageNightlyRate']['min'] }}-${{ $pricingStats['averageNightlyRate']['max'] }} per night across all categories, with significant variation by star rating, location, and season. 
                    @if($pricingStats['priceRange']['min'] > 0 || $pricingStats['priceRange']['max'] > 0)
                    Budget travelers can find quality options starting from ${{ number_format($pricingStats['priceRange']['min']) }}, while luxury experiences reach ${{ number_format($pricingStats['priceRange']['max']) }}+ per night.
                    @endif
                    </p>
                    @endif
                    ...
                    @php
                        ...
                        if (isset($pricingStats['totalHotels']) && $pricingStats['totalHotels'] > 0) {
                            $showMarketStats = true;
                        } elseif (isset($pricingStats['averageNightlyRate']) && 
                                ($pricingStats['averageNightlyRate']['min'] > 0 || $pricingStats['averageNightlyRate']['max'] > 0)) {
                            $showMarketStats = true;
                        } elseif (isset($pricingStats['priceRange']) && 
                                ($pricingStats['priceRange']['min'] > 0 || $pricingStats['priceRange']['max'] > 0)) {
                            $showMarketStats = true;
                        }
                    @endphp
                    ...
                    <ul style="list-style-type: disc; margin-left: 20px; margin-bottom: 20px;">
                        ...
                        @endif
                        @if(isset($pricingStats['averageNightlyRate']) && 
                           ($pricingStats['averageNightlyRate']['min'] > 0 || $pricingStats['averageNightlyRate']['max'] > 0))
                        ...
                        @endif
                        @if(isset($pricingStats['priceRange']) && 
                           ($pricingStats['priceRange']['min'] > 0 || $pricingStats['priceRange']['max'] > 0))
                    ...
                    @php
                        $hasValidRatings = false;
                        for ($i = 1; $i <= 5; $i++) {
                            if (isset($pricingStats['byStarRating'][$i]['averageRate']) && 
                                ($pricingStats['byStarRating'][$i]['averageRate']['min'] > 0 || 
                                 $pricingStats['byStarRating'][$i]['averageRate']['max'] > 0)) {
                    ...
                    @if(isset($pricingStats['byStarRating'][5]) && isset($pricingStats['byStarRating'][5]['averageRate']) && ($pricingStats['byStarRating'][5]['averageRate']['min'] > 0 || $pricingStats['byStarRating'][5]['averageRate']['max'] > 0))
                    ...
                    @if(isset($pricingStats['byStarRating'][4]) && isset($pricingStats['byStarRating'][4]['averageRate']) && ($pricingStats['byStarRating'][4]['averageRate']['min'] > 0 || $pricingStats['byStarRating'][4]['averageRate']['max'] > 0))
                    ...
                    @if(isset($pricingStats['byStarRating'][3]) && isset($pricingStats['byStarRating'][3]['averageRate']) && ($pricingStats['byStarRating'][3]['averageRate']['min'] > 0 || $pricingStats['byStarRating'][3]['averageRate']['max'] > 0))
                    ...
                    @if(isset($pricingStats['byStarRating'][2]) && isset($pricingStats['byStarRating'][2]['averageRate']) && ($pricingStats['byStarRating'][2]['averageRate']['min'] > 0 || $pricingStats['byStarRating'][2]['averageRate']['max'] > 0))
                    ...
                    @if(isset($pricingStats['byStarRating'][1]) && isset($pricingStats['byStarRating'][1]['averageRate']) && ($pricingStats['byStarRating'][1]['averageRate']['min'] > 0 || $pricingStats['byStarRating'][1]['averageRate']['max'] > 0))
                    ...
                    <p style="margin-top: 10px; margin-bottom: 15px;">{{ $location_info->Name }} offers exceptional hotel diversity across all price points, from ${{ $pricingStats['priceRange']['min'] }} budget motels to ${{ $pricingStats['priceRange']['max'] }}+ luxury resorts. Smart travelers can find quality accommodations by being flexible with location, timing, and amenities. Whether seeking Hollywood glamour, beachfront relaxation, or urban exploration, {{ $location_info->Name }}'s hotel market provides options for every traveler and budget.</p>

                    <h4 style="font-weight: bold; margin-top: 30px; margin-bottom: 15px; font-size: 20px;">Pro Tip:</h4>
                    <p style="margin-top: 10px; margin-bottom: 15px;">Book Tuesday-Thursday stays in shoulder seasons for optimal value, and consider neighborhoods like Koreatown and East Hollywood for authentic {{ $location_info->Name }} experiences at budget-friendly rates.</p>
                    @endif
                </div>
            </div>
        </div>
    ...
    @if(!empty($location_info) && !empty($location_info->about_hotel))
    <div class="tr-home-page">
      <div class="container">
        <div class="row">
  			  <div class="tr-single-page">
  				  <div class="tr-about-us-home-section">
                        {!! $location_info->about_hotel !!}
                    </div>
  		    	</div>
		    </div>
       </div>
   ...
   <div class="tr-breadcrumb-section">
    ...
    @if((!empty($sight_info) && count($sight_info) > 0) || 
        ...
        <li>
            <a href="{{ route('hotel.list', [$getlocationexp[0]->slugid.'-'.strtolower($getlocationexp[0]->Slug)]) }}">
