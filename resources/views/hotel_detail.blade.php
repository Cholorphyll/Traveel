...
<link rel="preconnect" href="https://www.google-analytics.com">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const lcpImage = document.querySelector('.lcp-image');
            if (lcpImage) {
                lcpImage.setAttribute("fetchpriority", "high"); // ✅ Ensure LCP prioritized
                lcpImage.setAttribute("loading", "eager");      // ✅ Prevent lazy-load delay
            }
    ...
    <title id="page-title">
        {{ e($searchresult[0]->name) }} in {{ e($searchresult[0]->CityName) . ', ' }} 
        @if (!empty($locationPatent))
            {{ e(last($locationPatent)['Name']) }}
        @elseif(!$getcontlink->isEmpty())
            {{ e($getcontlink[0]->Name) }}
        @endif - Best Prices 
        @if ($minprice > 0)
    ...
    <meta name="description" id="meta-description"
        content="@if (!empty($searchresult) && !empty($searchresult[0]->MetaTagDescription)) {{ $searchresult[0]->MetaTagDescription }}@elseif(!empty($searchresult))Discover {{ $searchresult[0]->name }} in {{ $searchresult[0]->cityName }}, featuring luxury rooms, central location, amenities. Compare prices across multiple booking platforms to find the best deal for your stay. Book smarter and save on your next trip.@else Discover great hotels with the best deals and amenities for your perfect stay. @endif">
    ...
    @php
        $currentHotelIdForImage = null;
        if (isset($searchresult, $searchresult[0]) && is_object($searchresult[0]) && property_exists($searchresult[0], 'hotelid')) {
            $currentHotelIdForImage = $searchresult[0]->hotelid;
        }

        $firstImageId = null;
        if ($currentHotelIdForImage && isset($images[$currentHotelIdForImage]) && is_array($images[$currentHotelIdForImage]) && count($images[$currentHotelIdForImage]) > 0) {
            $firstImageId = $images[$currentHotelIdForImage][0];
        }
    @endphp
    @if ($firstImageId)
      
        <link rel="preload" as="image"
              href="https://photo.hotellook.com/image_v2/limit/{{ $firstImageId }}/412/344.webp"
 ...
 <script>
        // Optimized version of your LCP loading script
        document.addEventListener('DOMContentLoaded', function() {
  ...
  "@graph": [
    {
      ...
      "image": [
        @if(isset($images[$searchresult[0]->hotelid][0]) && $images[$searchresult[0]->hotelid][0] != "")
          "https://photo.hotellook.com/image_v2/limit/{{ $images[$searchresult[0]->hotelid][0] }}/628/567.auto"
        @endif
        @if(isset($images[$searchresult[0]->hotelid][1]) && $images[$searchresult[0]->hotelid][1] != "")
          ,"https://photo.hotellook.com/image_v2/limit/{{ $images[$searchresult[0]->hotelid][1] }}/628/567.auto"
        @endif
        @if(isset($images[$searchresult[0]->hotelid][2]) && $images[$searchresult[0]->hotelid][2] != "")
          ,"https://photo.hotellook.com/image_v2/limit/{{ $images[$searchresult[0]->hotelid][2] }}/628/567.auto"
        @endif
        @if(isset($images[$searchresult[0]->hotelid][3]) && $images[$searchresult[0]->hotelid][3] != "")
          ,"https://photo.hotellook.com/image_v2/limit/{{ $images[$searchresult[0]->hotelid][3] }}/628/567.auto"
        @endif
        @if(isset($images[$searchresult[0]->hotelid][4]) && $images[$searchresult[0]->hotelid][4] != "")
          ,"https://photo.hotellook.com/image_v2/limit/{{ $images[$searchresult[0]->hotelid][4] }}/628/567.auto"
        @endif
      ],
      @endif
      "address": {
    ...
    $hotelQuestions = DB::table('HotelQuestion')
        ->select('CreatedDate','Question','updatedOn','Listing','Answer','faq_image')
    ...
    ,{
      ...
      "isPartOf": {
        "@type": "WebSite",
        "@id": "https://www.travell.co.in/#website"
      },
      "breadcrumb": {
        "@id": "{{ url()->current() }}#breadcrumb"
      }
    },
    {
      ...
      "@id": "https://www.travell.co.in/#website",
      "name": "Travell",
      "url": "https://www.travell.co.in"
    },
    {
      "@type": "Organization",
      "@id": "https://www.travell.co.in/#organization",
      "name": "Travell",
      "url": "https://www.travell.co.in",
      "logo": "https://www.travell.co.in/images/logo.png"
    }
  ]
}
...
<body>
            ...
            <div class="col-sm-12">
                <div class="tr-hotel-informations ">
                    <div class="tr-hotel-info">
                        <h1 class="tr-hotel-name aaa">{{ $searchresult[0]->name }}</h1>
                        ...
                        <div class="tr-raiting">
                            @if ($searchresult[0]->rating != '' && $searchresult[0]->rating != 0)
                                ...
                                ?>
                                <a href="javascript:void(0);" class="tr-excellent">{{ $result }}%
                                    <span style="color:{{ $color }}">{{ $ratingtext }}</span></a>
                            @endif
                        ...
                        <div class="tr-desktop d-none d-sm-block d-md-block" id="hotel-galary-image">
                            @if (!empty($searchresult[0]->hotelid))
                                ...
                                <div class="tr-main-image">
                                    @if (isset($images[$searchresult[0]->hotelid][0]) && $images[$searchresult[0]->hotelid][0] != '')
                                        <img src="https://photo.hotellook.com/image_v2/limit/{{ $images[$searchresult[0]->hotelid][0] }}/628/567.auto"
                                            alt="Room Image" style="height: 440px;width: 100%;" loading="lazy">
                                    @else
                                        <img src="{{ asset('/images/Hotel lobby-image.png') }}" alt=""
                                ...
                                <div class="tr-thumb-images">
                                    <ul>
                                        @php
                                            $remainingImages = isset($images[$hoteid])
                                                ? array_slice($images[$hoteid], 1)
                                                : [];
                                        @endphp

                                        @php
                                            $maxImages = 6; // Maximum number of thumbnail images to show
                                            $displayedImages = 0;
                                        @endphp
                                        @foreach ($remainingImages as $index => $image)
                                            @if($displayedImages < $maxImages)
                                                <li>
                                                    @if (!empty($image))
                                                        <img src="https://photo.hotellook.com/image_v2/limit/{{ $image }}/312/234.auto"
                                        ...
                                        @for($i = $displayedImages; $i < $maxImages; $i++)
                                            <li>
    ...
    @if (isset($images[$searchresult[0]->hotelid][0]) && $images[$searchresult[0]->hotelid][0] != '')
        ...
        <link rel="preload" 
              as="image" 
              href="https://photo.hotellook.com/image_v2/limit/{{ $images[$searchresult[0]->hotelid][0] }}/412/344.webp" 
              type="image/webp">

    <picture>
  <source srcset="https://photo.hotellook.com/image_v2/limit/{{ $images[$searchresult[0]->hotelid][0] }}/412/344.webp" type="image/webp">
  <source srcset="https://photo.hotellook.com/image_v2/limit/{{ $images[$searchresult[0]->hotelid][0] }}/412/344.jpg" type="image/jpeg">
  <img 
      src="https://photo.hotellook.com/image_v2/limit/{{ $images[$searchresult[0]->hotelid][0] }}/412/344.jpg"
      alt="Room Image"
      width="412"
      height="344"
      fetchpriority="high"
    ...
    @endif
                                            ...
                                            @if ($i < count($remainingImages))
                                                ...
                                                @if (!empty($image))
                                                    <div class="carousel-item">
                                                        <img src="https://photo.hotellook.com/image_v2/limit/{{ $image }}/412/344.auto"
                                                            alt="" class="d-block w-100" height="344"
                  ...
                  <img src="{{ asset('/frontend/hotel-detail/images/showimg.webp') }}" alt="">
                                    
                    <p>Show all 12+</p>
                  </div>
                                    <div class="tr-thumbs-count tr-show-all-photos">
                                    Show all 122+
                                    </div>
                                </div>
                            </div>
            ...
            <div class="col-lg-7 col-md-12 tr-left-col">
                        ...
                        <div class="tr-hotel-certificated">
                            <div class="row" style="display: flex;">
                                ...
                                <div class="tr-families-favourite">
                                    <img src="{{ asset('/frontend/hotel-detail/images/leaf.png') }}" alt="leaf"  loading="lazy"
             decoding="async"
             fetchpriority="low"
             onload="this.classList.add('loaded')">
                                    <span>Families Favourite</span>
                                    <p>{{ $searchresult[0]->name }} is praised for its great location near the city
                                        center and attractions in {{ $searchresult[0]->CityName }}. Guests
                                        appreciate the convenient and easy access to nearby exclusive all-suite hotels
                                        providing 4-star
                                        facilities...</p>
                                        </div>
                                </div>
                                ...
                                <div class="col-md-6" style="flex: 1; display: flex; flex-direction: column; gap:15px">
                                    <!-- Spotlight Section -->
                                    @if (!empty($searchresult[0]->Spotlights))
                                        <div class="tr-spotlight" style="flex: 1; margin-bottom: 0px;">
                                            <div class="tr-heading">
                                                <svg width="24" height="7" viewBox="0 0 24 7" fill="none"
                                                    xmlns="http://www.w3.org/2000/svg">
                                                    <path
                                                        d="M23.3868 3.5L20.5 0.613249L17.6132 3.5L20.5 6.38675L23.3868 3.5ZM0.5 4H20.5V3H0.5V4Z"
                                                        fill="#09707A" />
                                                </svg>
                                                ...
                                                <svg width="24" height="7" viewBox="0 0 24 7" fill="none"
                                                    xmlns="http://www.w3.org/2000/svg">
                                                    <path
                                                        d="M0.613249 3.5L3.5 0.613249L6.38675 3.5L3.5 6.38675L0.613249 3.5ZM23.5 4H3.5V3H23.5V4Z"
                                                        fill="#09707A" />
                                                </svg>
                                            </div>
                                            <div class="tr-heading__bottom">{{ $searchresult[0]->Spotlights }}</div>
                                        </div>
                                    @endif

                                    <!-- Great For Section -->
                                    @if (
                                        $searchresult[0]->GreatFor != '' ||
                                            ($searchresult[0]->GreatForScore != '' && $searchresult[0]->GreatForScore != 0.0))
                                        <div class="tr-business-rated" style="flex: 1;">
                                            <div class="tr-title">Great for</div>
                                            <div class="tr-business">
                                                @if ($searchresult[0]->GreatFor != '')
                                                    {{ $searchresult[0]->GreatFor }}
                                                @endif
                                            </div>
                                            <div
                                                class="@if ($searchresult[0]->GreatFor != '') tr-rated @else tr-business @endif">
                                                @if ($searchresult[0]->GreatForScore != '' && $searchresult[0]->GreatForScore != 0.0)
                                                    Rated {{ $searchresult[0]->GreatForScore }}
                                                @endif
                                            </div>
                                            <img src="{{ asset('/frontend/hotel-detail/images/icons/marketeq-icon.svg') }}"
                                                alt=""  loading="lazy"
             decoding="async"
             fetchpriority="low"
             onload="this.classList.add('loaded')">
                    ...
                    <div class="tr-overview-section tr-tab-section" id="overviewTab">
                                        ...
                                        <ul class="first-section">
                                            @foreach (array_slice($filteredHighlights, 0, 4) as $highlight)
                                                <li style="background-color: white;">
                                                    <img src="{{ asset('/frontend/hotel-detail/images/amenities/' . getHighlightIcon($highlight)) }}"
                                                        alt="{{ $highlight }}" style="width: 24px; height: 24px;">
                                                    ...
                                                    <li style="background-color: white;">
                                                        <svg width="25" height="25" viewBox="0 0 25 25"
                                                            ...
                                                            <path
                                                                ...
                                                                stroke="#222222" stroke-width="1.44"
                        ...
                        @if (!empty($searchresult[0]->about) || !empty($searchresult[0]->OverviewShortDesc))
                            <div class="tr-things-know">
                                <h3>About</h3>
                                <ul class="thingsToKnowContent1"
                                    style="text-align: justify; list-style-position: inside; max-height: 100px; overflow: hidden;">
                                    @php
                                    ...
                                    @if (is_string($overviewContent))
                                        ...
                                        @foreach ($overviewArray as $item)
                                            @if (!empty(trim($item)))
                                                <!-- Avoid displaying empty items -->
                                                <li><img src="{{ asset('/frontend/hotel-detail/images/amenities/circle.svg') }}"
                                                        alt="icon" class="Icon"
                                                        style="margin-right: 10px;">{{ trim($item) }}.</li>
                                            @endif
                                        @endforeach
                                    @endif
                                </ul>
                                <button id="toggleButton1" onclick="toggleReadMore1()" class="tr-anchor-btn"
                        ...
                        @if (!empty($searchresult[0]->knownfor))
                            <div class="tr-overview-items" style="margin-top: 40px;">
                                <div class="tr-overview-items-icon">
                                    <img src="images/icons/overview-star-icon.svg" alt=""  loading="lazy"
             decoding="async"
             fetchpriority="low"
             onload="this.classList.add('loaded')">
                                </div>
                        ...
                        @if (!empty($searchresult[0]->AmenitiesRest))
                            <div class="tr-overview-items">
                                <div class="tr-overview-items-icon">
                                    <img src="images/icons/overview-beverages-icon.svg" alt=""  loading="lazy"
             decoding="async"
             fetchpriority="low"
             onload="this.classList.add('loaded')">
                                </div>
                        ...
                        @if (!empty($searchresult[0]->PlaceKnownFor))
                            <div class="tr-overview-items">
                                <div class="tr-overview-items-icon">
                                    <img src="images/icons/overview-compass-icon.svg" alt=""  loading="lazy"
             decoding="async"
             fetchpriority="low"
             onload="this.classList.add('loaded')">
                                </div>
                        ...
                        @if (!empty($searchresult[0]->PriceRange))
                            <div class="tr-overview-items">
                                <div class="tr-overview-items-icon">
                                    <img src="images/icons/overview-dollar-icon.svg" alt=""  loading="lazy"
             decoding="async"
             fetchpriority="low"
             onload="this.classList.add('loaded')">
                                </div>
             ...
             onload="this.classList.add('loaded')">
                        ...
                        @if (!empty($searchresult[0]->Moreinfo))
                            <div class="tr-overview-items">
                                <div class="tr-overview-items-icon">
                                    <img src="images/icons/overview-info-icon.svg" alt=""  loading="lazy"
             decoding="async"
             fetchpriority="low"
             onload="this.classList.add('loaded')">
                                </div>
                                ...
                                <ul id="thingsToKnowContent"
                                    ...
                                    @elseif (is_string($searchresult[0]->ThingstoKnow))
                                        ...
                                        @foreach ($thingsArray as $thing)
                                            @if (!empty(trim($thing)))
                                                <li><img src="{{ asset('/frontend/hotel-detail/images/amenities/circle.svg') }}"
                                                        alt="icon" class="Icon"  loading="lazy"
             decoding="async"
             fetchpriority="low"
             onload="this.classList.add('loaded')"> {{ trim($thing) }}.</li>
                                            @endif
                                        @endforeach
                                    @endif
                                    ...
                                    @elseif (is_string($searchresult[0]->Children_Policy))
                                        @php
                                            $thingsArray = preg_split(
                                                '/\r\n|\r|\n/',
                                                $searchresult[0]->Children_Policy,
             ...
             decoding="async"
             fetchpriority="low"
             onload="this.classList.add('loaded')"> <strong>Smoking Policy:</strong>
             ...
             onload="this.classList.add('loaded')"> {{ trim($thing) }}.</li>
                                    ...
                                    @elseif (is_string($searchresult[0]->QuiteHours_Policy))
                                        @php
                                            $thingsArray = preg_split(
                                                '/\r\n|\r|\n/',
                                                $searchresult[0]->QuiteHours_Policy,
                                            ); // Split by line breaks
                                        @endphp
                                        @foreach ($thingsArray as $thing)
                                            @if (!empty(trim($thing)))
            ...
            <div class="col-lg-5 col-md-12 tr-right-col">
             ...
             onload="this.classList.add('loaded')">
                    ...
                    <div class="tr-spotlight">
                        <div class="tr-heading"><svg width="24" height="7" viewBox="0 0 24 7"
                                fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M23.3868 3.5L20.5 0.613249L17.6132 3.5L20.5 6.38675L23.3868 3.5ZM0.5 4H20.5V3H0.5V4Z"
                                    fill="#09707A" />
                            </svg>IN THE SPORTLIGHT<svg width="24" height="7" viewBox="0 0 24 7"
                                fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M0.613249 3.5L3.5 0.613249L6.38675 3.5L3.5 6.38675L0.613249 3.5ZM23.5 4H3.5V3H23.5V4Z"
                                    fill="#09707A" />
                    ...
                    <div class="tr-business-rated">
                        ...
                        <img src="{{ asset('/frontend/hotel-detail/images/icons/marketeq-icon.svg') }}" alt=""  loading="lazy"
             decoding="async"
             fetchpriority="low"
             onload="this.classList.add('loaded')"> 
                    </div>
                ...
                $guest = isset($sessionData["guest"])
                    ? $sessionData["guest"]
                    : "";
                $rooms = isset($sessionData["rooms"])
                    ? $sessionData["rooms"]
                    : "";
                ?>
                <div class="tr-booking-deatils">
                    <div class="tr-date-section section-2">
                        <label class="tr-room-guest"> <span class="room-count">
                                @if ($rooms != '')
                                    {{ $rooms }}
                                @else
                                    1
                                @endif
                            </span> room , <span class="guest-count">
                                @if ($guest != '')
                                    {{ $guest }}
                                @else
                                    2
                            ...
                            <div class="tr-guests-modal">
                                <div class="tr-add-edit-guest tr-total-num-of-rooms">
                                    ...
                                    <div class="tr-qty-box">
                                        ...
                                        <input type="text" id="totalRoom" class="totalRoom"
                                            value="@if ($rooms != '') {{ $rooms }}@else 1 @endif"
                                            id="" min="1" max="10" name="" readonly />
                                ...
                                <div class="tr-add-edit-guest tr-total-guest">
                                    ...
                                    <div class="tr-qty-box">
                                        <button class="minus disabled" value="minus">-</button>
                                        <input type="text" id="totalAdultsGuest"
                                            value="@if ($guest != '') {{ $guest }}@else 2 @endif"
                                            id="" min="1" max="10" name="" readonly />
                                        <button class="plus" value="plus">+</button>
                                    </div>
                                </div>
                                <div class="tr-add-edit-guest tr-total-children">
                                    ...
                                    <div class="tr-qty-box">
                                        <button class="minus disabled" value="minus">-</button>
                                        <input type="text" id="totalChildrenGuest" value="0" id=""
                                            min="1" max="10" name="" readonly />
                                        <button class="plus" value="plus">+</button>
                                    </div>
                                </div>
                                <div class="tr-add-edit-guest tr-total-infants">
                                    ...
                                    <div class="tr-qty-box">
                                        <button class="minus disabled" value="minus">-</button>
                                        <input type="text" id="totalChildrenInfants" value="0"
                                            id="" min="1" max="10" name="" readonly />
                                        <button class="plus" value="plus">+</button>
                                    </div>
                                </div>
                            ...
                            <div class="tr-quick-search">
                                <div class="tr-stay-date">
                                    <div class="tr-col">
                                        ...
                                        <input type="text"
                                            @if (Session::has('checkin')) value="{{ $checkin }}" @endif
                                            id="checkInSelectDate" class="checkinval2 checkin" name=""
                                            placeholder="select date" name="" autocomplete="off">
                                    </div>
                                    <div class="tr-col">
                                        <label>Check-out</label>
                                   
                                        <input type="text"
                                            @if (Session::has('checkin')) value="{{ $checkout }}" @endif
                                            id="checkOutSelectDate" class="checkoutval2 checkout" name=""
                                            placeholder="select date" name="" autocomplete="off">
                                    </div>
                                </div>
                                <button type="submit" class="tr-btn filterchackinout">Get Price</button>
                                <!-- <span class="d-none price-loader"> -->
                                <button type="submit" class="tr-btn is-loading d-none price-loader">Get
                                    Price</button>
                            ...
                            <div class="tr-calenders-modal-2">
                                <div class="tr-calenders-top">
                                    <div class="tr-search-info">
                                        ...
                                        <div class="tr-check-in-checkout-date" id="totaldates">
                                    ...
                                    <div class="tr-stay-date">
                                        <div class="tr-col">
                                            ...
                                            <input type="text" id="checkInInput2"
                                                class="checkIn dateInput checkinval2"
                                                @if (Session::has('checkin')) value="{{ $checkin }}" @endif
                                                placeholder="Select a date" name="" readonly
                                                autocomplete="off">
                                        </div>
                                        <div class="tr-col">
                                            <label for="checkOutInput2">Check-out</label>

                                            <input type="text" id="checkOutInput2"
                                                class="checkOut dateInput checkoutval2"
                                                @if (Session::has('checkin')) value="{{ $checkout }}" @endif
                                                placeholder="Select a date" name="" readonly
                                                autocomplete="off">
                                        </div>
                                ...
                                <div class="tr-action">
                                    <button type="button" class="tr-clear-details" id="reset2">Clear
                                        dates</button>
                            ...
                            @if (!$getprice->isEmpty())
                                @php
                                    ...
                                    foreach ($getprice as $room) {
                                        ...
                                        if (
                                            !isset($lowesttotals[$agencyName]) ||
                                            $total < $lowesttotals[$agencyName]['total']
                                        ) {
                                            $lowesttotals[$agencyName] = [
                                                'total' => $total,
                                ...
                                <div style="text-align: center;">
                                    @foreach ($uniquePrices as $index => $total)
                                        @php
                                            ...
                                            $isLowestPrice =
                                                isset($lowesttotals[$agencyData->agency_name]) &&
                                                $total == $lowesttotals[$agencyData->agency_name]['total'];
                                        ...
                                        @if ($index == 0)
                                            ...
                                            <div class="tr-travel-site">
                                                <div class="tr-travel-img">
                                                    ...
                                                    <img loading="lazy"
                                                        src="{{ 'http://pics.avs.io/hl_gates/100/30/' . $agencyData->agency_id . '.png' }}"
                                                        alt="Booking">
                                                </div>
                                                <div class="tr-travel-price">
                                                    <a href="{{ $agencyData->booking_link }}"
                                                        target="_blank"><strong>${{ $total }} </strong>
                                                        /night</a>
                                                </div>
                                            </div>
                                        @elseif ($index < 3)
                                            ...
                                            <div class="tr-travel-site" style="border-top: 1px solid #E9E9E9;">
                                                <div class="tr-travel-img">
                                                    <img src="{{ 'http://pics.avs.io/hl_gates/100/30/' . $agencyData->agency_id . '.png' }}"
                                                        alt="Booking">
                                                </div>
                                                <div class="tr-travel-price">
                                                    <a href="{{ $agencyData->booking_link }}"
                                ...
                                @if ($showMore)
                                    ...
                                    <div class="tr-show-all">
                                        <button id="showMoreDeal" type="button"
                                            value="More">{{ $remainingDeals }} more deals from
                                            ${{ $latval }}</button>
                                        <button id="showLessDeal" type="button" value="Less" class="d-none">Show
                                            less deals from ${{ $latval }}</button>
                                    </div>
                                @endif
                            @else
                                @if (empty($prices) && !empty(request()->get('checkin')) && !empty(request()->get('checkout')))
                                    @if (!isset($soldOutDisplayed))
                                        ...
                                        <div class="tr-sold-out">
                                            <img src="{{ asset('/frontend/hotel-detail/images/sold-out.png') }}"
                                                alt="Sold Out">
                                ...
                                @foreach ($amenitiesListroom as $key => $value)
                                    @if ($value != 'doublebed' && $value != 'twin')
                                        <div class="tr-filter-list filter_hotel_room">
                                            <input type="checkbox" name="values[]" id="filter1-{{ $value }}"
                                                class="filter allamenities {{ $value }}"
                                                data-value="{{ $value }}" value="{{ $value }}">
                                            <label for="filter1-{{ $value }}">
                                                <i class="{{ fontArray($value) }}"></i>
                                                {{ strip_tags(getAmenitiesNames($value)) }}
                                            </label>
                                        </div>
                                    @endif
                                @endforeach
                                ...
                                <div class="tr-hotel-deatils">
                                    ...
                                    <div class="tr-hotal-image">
                                        <div id="roomSlider{{ $loop->index }}">
                                            <img loading="lazy"
                                                src="https://photo.hotellook.com/image_v2/limit/h{{ $roomEntries[0]->hotelid }}_0/300.jpg"
                                                alt="Room Image" height="203" width="296">
                                        </div>
                                    </div>
                                        ...
                                        <ul class="tr-desktop">
                                            @foreach ($amenitiesListroom as $amenity)
                                                @if (isset($roomEntries[0]->$amenity) && $roomEntries[0]->$amenity == 1)
                                                    <li><i class="Roomamenities {{ fontArray($amenity) }}"></i>
                                                        {{ strip_tags(getAmenitiesNames($amenity)) }}</li>
                                                @endif
                                            @endforeach
                                        </ul>
                                    </div>
                                    ...
                                    <div class="tr-hotel-price-lists">
                                        @foreach (array_slice($roomEntries, 0, 5) as $roomEntry)
                                            <div class="tr-hotel-price-list">
                                                ...
                                                <div class="tr-site-details">
                                                    ...
                                                    @if (!empty($agencyImage))
                                                        <img loading="lazy"
                                                            src="{{ asset('agency/' . $agencyName . '.png') }}"
                                                            alt="{{ ucfirst($agencyName) }}" width="80"
                                                            height="40">
                                                    @endif
                                                    <div class="tr-hotel-facilities">
                                                        <ul>
...
?>
                <div class="tr-facilities-details">
                    @if (!empty($shortFacilities))
                        ...
                        foreach ($popamenities as $amenity) {
                            $match = $shortFacilities->firstWhere(
                                "name",
                                $amenity
                            );
                            if (
                                $match &&
                                !$matchingFacilities->contains(
                                    "name",
                                    $match->name
                                )
                            ) {
                        ...
                        $nonMatchingFacilities = $shortFacilities->reject(
                            function ($facility) use ($popamenities) {
                                return in_array(
                                    $facility->name,
                                    array_keys($popamenities)
                                );
                            }
                        );

                        // Combine collections, keeping the order specified in $popamenities
                        $sortedFacilities = $matchingFacilities->merge(
                            $nonMatchingFacilities
                        );
                        ?>
                        ...
                        <ul class="tr-facilities-box-lists">
                            @foreach ($sortedFacilities as $mnt)
                                <li>
                                    ...
                                    @endphp
                                    <img src="{{ asset('/frontend/hotel-detail/images/amenities/' . $iconName . '.svg') }}"
                                        alt="{{ $mnt->name }}">
                ...
                <div class="tr-facilities-lists tr-desktop">
                    ...
                    @if (!$facilityNames->isEmpty())
                        ...
                        @endphp
<div class="tr-facilities-wrapper">
        ...
        <div class="tr-Location-section tr-tab-section" id="locationTab">
            ...
            <div class="tr-Location-details">
                <ul>
                    @if ($searchresult[0]->address != '')
                        <li class="tr-location-address">{{ $searchresult[0]->address }}
                        </li>
                    @endif
                    <li class="tr-phone">
                        @if (!empty($searchresult[0]->Phone))
                    ...
                    <li class="tr-website-link"><a
                            href="@if (!empty($searchresult[0]->Website)) {{ $searchresult[0]->Website }} @endif"
            ...
            <div class="tr-location-map">
                @if ($searchresult[0]->Latitude != '' && $searchresult[0]->longnitude != '')
                    ...
                    @php
                        ...
                        $nearbySights = DB::table('Tripalong.TPhotel_neaby_sight')->where('hotelid', $hotelId)->get();
                    @endphp
                    <script>
            ...
            <div class="tr-tourist-places-row">
                <div class="tr-tourist-places">
                    <div class="tr-title open">
                        <h4><svg width="25" height="25" viewBox="0 0 25 25" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path
                        ...
                        <span class="d-none d-md-block">Sorted by nearest @if (!$nearby_sight->isEmpty())
                                within
                                {{ $nearby_sight[0]->radius }} km
                            @endif
                        </span>
                    </div>
                ...
                <div class="tr-tourist-places" id="nearbyrest">
                    <div class="tr-title">
                        <h4><svg width="24" height="20" viewBox="0 0 19 19" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path
                ...
                <!-- end data rest -->
                <div class="tr-tourist-places" id="transportation-section">
                    <div class="tr-title">
                        <h4><svg width="25" height="25" viewBox="0 0 25 25" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path
                        ...
                        <span class="d-none d-md-block">Transportation options</span>
                    </div>
                    <div class="tr-tourist-place-lists">
                    ...
                    @foreach ($get_experience as $get_experiences)
                        <div class="tr-top-ways-list">
                            ...
                            <div class="tr-details">
                                ...
                                <div class="tr-like-review">
                                    <span class="tr-likes"><svg width="14" height="13" viewBox="0 0 14 13"
                                            fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div class="row">
                <div class="col-sm-12">
                    ...
                    <div class="tr-show-all-review">
                        <button type="button" class="tr-show-all-review-btn" data-toggle="modal"
                            data-target="#highlight-review-modal">Add Highlight Review</button>
                    </div>
                </div>
            </div>
        </div>
...
<section class="faq__Section">
    <div class="container">
        <h4> Q & A for {{ $searchresults[0]->name }}</h4>

        <div class="row g-4">
            @if(isset($hotelQuestionAnswer['grouped_faqs']) && $hotelQuestionAnswer['grouped_faqs']->count() > 0)
                @php
                    $allFaqsForDisplay = collect();
                    foreach ($hotelQuestionAnswer['grouped_faqs'] as $cat => $list) {
                        $allFaqsForDisplay = $allFaqsForDisplay->merge($list);
                    }
                    $allFaqsForDisplay = $allFaqsForDisplay->take(10);
                @endphp

                @if($allFaqsForDisplay->count() > 5)
                    <div class="col-md-8">
                @else
                    <div class="col-md-4">
                @endif
                    <div class="row">
                        @if($allFaqsForDisplay->count() > 0)
                            <!-- First column -->
                            <div class="{{ $allFaqsForDisplay->count() > 5 ? 'col-md-6' : 'col-md-12' }}">
                                <div class="faq-card">
                                    @foreach ($allFaqsForDisplay->slice(0, 5) as $faq)
                                        <div class="faq-item" data-bs-toggle="offcanvas" data-bs-target="#faqSidebarSearch" onclick="showSpecificFaq('{{ $faq->HotelQuestionId }}')">
                                            <span><img src="{{ asset('explore/images/icons/chat-alt-2.svg') }}" alt="Question Icon" /></span>
                                            <div class="faq-text">{{ $faq->Question }}</div>
                                            <span><img src="{{ asset('explore/images/icons/arrowrightnew.svg') }}" alt="Arrow Icon" /></span>
                                        </div>
                                    @endforeach
                                </div>
                            </div>

                            <!-- Second column if there are more than 5 FAQs -->
                            @if($allFaqsForDisplay->count() > 5)
                            <div class="col-md-6">
                                <div class="faq-card">
                                    @foreach ($allFaqsForDisplay->slice(5, 5) as $faq)
                                        <div class="faq-item" data-bs-toggle="offcanvas" data-bs-target="#faqSidebarSearch" onclick="showSpecificFaq('{{ $faq->HotelQuestionId }}')">
                                            <span><img src="{{ asset('explore/images/icons/chat-alt-2.svg') }}" alt="Question Icon" /></span>
                                            <div class="faq-text">{{ $faq->Question }}</div>
                                            <span><img src="{{ asset('explore/images/icons/arrowrightnew.svg') }}" alt="Arrow Icon" /></span>
           ...
           @endif
            ...
            <div class="col-md-4 {{ !(isset($hotelQuestionAnswer['grouped_faqs']) && $hotelQuestionAnswer['grouped_faqs']->count() > 0) ? 'ms-auto' : '' }}">
                <div class="faq-card">
                    <div class="faq__innerSection">
                        <h2>Still looking?</h2>
                        <button data-bs-toggle="offcanvas" data-bs-target="#faqSidebar">
                            Still looking?
                        </button>
        ...
        @if(isset($hotelQuestionAnswer['grouped_faqs']) && $hotelQuestionAnswer['grouped_faqs']->count() > 0)
        <div class="viewAll" data-bs-toggle="offcanvas" data-bs-target="#faqSidebarSearch">
            <span>View all <img src="{{ asset('explore/images/icons/arrowrightnew.svg') }}" alt="Arrow Icon"></span>
        </div>
        @endif
    </div>
</section>
...
<div class="offcanvas custom-offcanvas sidebaar__faq" tabindex="-1" id="faqSidebar" aria-labelledby="faqSidebarLabel">
    <div class="offcanvas-header">
        <h5>Q & A for {{ $searchresults[0]->name }}</h5>
        <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
    </div>
    <div class="offcanvas-body d-flex flex-column">
        <p class="bodySection"><strong>About:</strong> {{ $searchresults[0]->name }}</p>
        <form id="askQuestionForm" class="slidebar__inner flex-grow-1 d-flex flex-column">
            @csrf
            <input type="hidden" name="TPHotel_id" value="{{ $searchresults[0]->id }}">
            <div class="question">
                <span class="important__question">Your question</span>
                <span id="charCount">0/300</span>
            </div>
            <textarea id="question" name="question" class="form-control" rows="4" maxlength="300" placeholder="e.g. do you offer room service?"></textarea>
            <div class="formtext">300 Characters left</div>
            <div class="infoSection">
                <img src="{{ asset('explore/images/icons/infoicon.svg') }}" alt="Info Icon" />
                <p>If we can't answer your question right away, you can forward it to the property.</p>
            </div>
            <div id="questionResponse" class="alert mt-2" style="display: none;"></div>
            <div class="mt-auto pt-3">
                <button type="button" class="ask__question" id="submitQuestion">Ask a question</button>
            </div>
        </form>
    </div>
</div>
...
<!-- All FAQs Sidebar -->
<div class="offcanvas custom-offcanvas sidebaar__faq body-scrollable" tabindex="-1" id="faqSidebarSearch" aria-labelledby="faqSidebarLabel">
    <div class="offcanvas-body d-flex flex-column body__main">
        <div class="offcanvas-header">
            <h5>Q & A for {{ $searchresults[0]->name }}</h5>
            <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
        </div>
        <p class="bodySection"><strong>About:</strong> {{ $searchresults[0]->name }}</p>
        <div class="slidebar__inner flex-grow-1 d-flex flex-column">
            <div class="search-box">
                <input type="text" id="faqSearchInput" class="search-input" placeholder="Search questions or tags..." />
                <button class="search-button" id="faqSearchButton">
                    <img src="{{ asset('explore/images/icons/search.svg') }}" alt="Search Icon" />
                </button>
                <div id="searchResults" class="search-results" style="display: none;"></div>
            </div>
            @foreach ($hotelQuestionAnswer['all_faqs'] as $faq)
                <div class="info__main" data-faq-id="{{ $faq->HotelQuestionId }}" data-tags="{{ $faq->Tags ?? '' }}">
                    <div class="infoSection">
                        @if(!empty($faq->faq_image))
                            <img src="{{ asset($faq->faq_image) }}" alt="Question Icon" />
                        @else
                            <img src="{{ asset('explore/images/icons/chat-alt-2.svg') }}" alt="Question Icon" />
                        @endif
                        <p>{{ $faq->Question }}</p>
                    </div>
                    <div class="info__details">
                        <p class="date">{{ \Carbon\Carbon::parse($faq->CreatedDate)->format('d F Y') }}</p>
                        <div class="info__details__title_body">
                            @if (strlen($faq->Answer) > 100)
                                <span class="truncated-answer">{{ Str::limit($faq->Answer, 100) }}</span>
                                <span class="full-answer" style="display: none;">{{ $faq->Answer }}</span>
                                <span class="dots">...</span>
                                <a href="javascript:void(0);" class="show-more-toggle" onclick="toggleAnswer(this)">Show more</a>
                            @else
                    ...
                    <div class="like__button">
                        <a href="javascript:void(0);" class="like__section like-btn" onclick="voteFaq(this, {{ $faq->HotelQuestionId }}, 'like')" data-faq-id="{{ $faq->HotelQuestionId }}" data-action="like">
                            <span><img src="{{ asset('explore/images/icons/thumbs-up.svg') }}" alt="Thumbs Up" /></span>
                            <span>Helpful</span>
                            <span class="like-count">{{ isset($faq->Likes) && $faq->Likes > 0 ? $faq->Likes : '0' }}</span>
                        </a>
                        <a href="javascript:void(0);" class="like__section dislike-btn" onclick="voteFaq(this, {{ $faq->HotelQuestionId }}, 'dislike')" data-faq-id="{{ $faq->HotelQuestionId }}" data-action="dislike">
                            <span><img src="{{ asset('explore/images/icons/thumbs-down.svg') }}" alt="Thumbs Down" /></span>
    ...
    <div class="offcanvas-footer mt-auto">
        <button class="ask__question w-100" data-bs-toggle="offcanvas" data-bs-target="#faqSidebar">Ask a question</button>
    </div>
</div>
                        ...
                        <div class="tr-review-content mb-3">
                            ...
                            @if ($searchresult[0]->ReviewSummaryLabel != '')
                                <ul class="tr-revies-recomm">
                                    ...
                                    <?php
                                    $reviewSummaryLabel =
                                        $searchresult[0]->ReviewSummaryLabel;
                                    $expreviewSummaryLabel = explode(
                                        ",",
                                        $reviewSummaryLabel
                                    );
                                    $t = 1;
                    ...
                    @if (!$review->isEmpty())
                        <div class="tr-reviews-graph">
                            <div class="tr-reviews-graph-details">
                                ...
                                <div class="tr-likes-count"><svg width="17" height="15" viewBox="0 0 17 15"
                                        fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path fill-rule="evenodd" clip-rule="evenodd"
                                ...
                                <div class="tr-reviews-count">({{ $reviewStats['totalReviews'] }}
                                    {{ $reviewStats['totalReviews'] == 1 ? 'Review' : 'Reviews' }})</div>
                            </div>
                            <div class="tr-rating-types">
                                <div class="tr-rating-type">
                                    ...
                                    <div class="tr-reviews-nums">
                                        {{ number_format($reviewStats['avgCleanliness'], 1) }}</div>
                                </div>
                                <div class="tr-rating-type">
                                    <div class="tr-title">Location</div>
                                    <div class="progress">
                                        <div class="progress-bar" role="progressbar"
                                            aria-valuenow="{{ $reviewStats['avgLocation'] * 20 }}" aria-valuemin="0"
                                            aria-valuemax="100"
                                            style="width:{{ $reviewStats['avgLocation'] * 20 }}%">
                                        </div>
                                    </div>
                                    <div class="tr-reviews-nums">{{ number_format($reviewStats['avgLocation'], 1) }}
                                    </div>
                                </div>
                                <div class="tr-rating-type">
                                    ...
                                    <div class="progress">
                                        <div class="progress-bar" role="progressbar"
                                            aria-valuenow="{{ $reviewStats['avgService'] * 20 }}" aria-valuemin="0"
                                            aria-valuemax="100"
                                            style="width:{{ $reviewStats['avgService'] * 20 }}%">
                                        </div>
                                    </div>
                                    <div class="tr-reviews-nums">{{ number_format($reviewStats['avgService'], 1) }}
                                    </div>
                                </div>
                                <div class="tr-rating-type">
                                    ...
                                    <div class="progress">
                                        <div class="progress-bar" role="progressbar"
                                            aria-valuenow="{{ $reviewStats['avgValue'] * 20 }}" aria-valuemin="0"
                                            aria-valuemax="100" style="width:{{ $reviewStats['avgValue'] * 20 }}%">
                                        </div>
                                    </div>
                                    <div class="tr-reviews-nums">{{ number_format($reviewStats['avgValue'], 1) }}
                                    </div>
                                </div>
                            </div>
                        </div>
...
?>
                            ...
                            @if (!$review->isEmpty())
                                @foreach ($review as $reviews)
                                    <div class="tr-customer-review">
                                        ...
                                        <ul>
                                            <li><svg width="20" height="22" viewBox="0 0 20 22"
                                                    fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path
                                            ...
                                            <li><svg width="25" height="25" viewBox="0 0 25 25"
                                                    ...
                                                    <path
                                                        ...
                                                        stroke="#222222" stroke-width="1.28571"
                                                        stroke-linecap="round" stroke-linejoin="round">
                                                    </path>
                                                    <path d="M8.41406 5.48145V8.98144" stroke="#222222"
                                                        stroke-width="1.28571" stroke-linecap="round"
                                                        stroke-linejoin="round"></path>
                                                    <path d="M16.4141 5.48145V8.98144" stroke="#222222"
                                                        stroke-width="1.28571" stroke-linecap="round"
                                                        stroke-linejoin="round"></path>
                                                    <path d="M8.16406 11.4814H16.6641" stroke="#222222"
                                                        stroke-width="1.28571" stroke-linecap="round"
                                                        stroke-linejoin="round"></path>
                                                </svg>1 night. {{ date('F j', strtotime($reviews->CreatedOn)) }}</li>
                                            <li><svg width="20" height="21" viewBox="0 0 20 21"
                                                    fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path
                                                        d="M16.6615 17.8091V16.1424C16.6615 15.2584 16.3103 14.4105 15.6851 13.7854C15.06 13.1603 14.2122 12.8091 13.3281 12.8091H6.66146C5.7774 12.8091 4.92956 13.1603 4.30444 13.7854C3.67931 14.4105 3.32813 15.2584 3.32812 16.1424V17.8091"
    ...
    <picture>
        ...
        <!-- WebP fallback -->
        <source srcset="{{ asset('frontend/hotel-detail/images/no-review-found.webp') }}" type="image/webp">
        <!-- PNG fallback -->
    ...
    <p>Review not found</p>
            ...
            <div class="tr-house-rules tr-desktop">
                ...
                <div class="tr-rules-ques-ans">
                    ...
                    <div class="tr-rules-ans">
                        <div class="tr-rules-heading">
                            @if (!empty($searchresult[0]->checkIn))
                                {{ $searchresult[0]->checkIn }}
                            @else
                                Not Available.
                            @endif
                        </div>
                ...
                <div class="tr-rules-ques-ans">
                    ...
                    <div class="tr-rules-ans">
                        <div class="tr-rules-heading">
                            @if (!empty($searchresult[0]->checkOut))
                                {{ $searchresult[0]->checkOut }}
                            @else
                                Not Available.
                            @endif
                        </div>
              ...
              <div class="tr-rules-ans">
                ...
                <div class="tr-rules-ques-ans">
                    <div class="tr-rules-ques">Cards accepted at this hotel</div>
                    <div class="tr-rules-ans">
                        <svg width="224" height="30" viewBox="0 0 224 30" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <rect x="0.307692" y="0.616286" width="42.4615" height="28.9231" rx="3.38461"
                                fill="white" stroke="#D9D9D9" stroke-width="0.615385" />
                            <path fill-rule="evenodd" clip-rule="evenodd"
                                ...
                                fill="#172B85" />
                            <rect x="60.6153" y="0.616286" width="42.4615" height="28.9231" rx="3.38461"
                                fill="white" stroke="#D9D9D9" stroke-width="0.615385" />
                            <path fill-rule="evenodd" clip-rule="evenodd"
                            ...
                            <path fill-rule="evenodd" clip-rule="evenodd"
                                ...
                                fill="#FF5E00" />
                            <rect x="120.923" y="0.616286" width="42.4615" height="28.9231" rx="3.38461"
                                fill="#1F72CD" stroke="#D9D9D9" stroke-width="0.615385" />
                            <path fill-rule="evenodd" clip-rule="evenodd"
                                ...
                                fill="white" />
                            <rect x="181.231" y="0.616286" width="42.4615" height="28.9231" rx="3.38461"
                                fill="white" stroke="#D9D9D9" stroke-width="0.615385" />
                            <path fill-rule="evenodd" clip-rule="evenodd"
                        ...
                        @if ($nearbhotel->id != $searchresult[0]->id)
                            <div class="tr-nearby-hotel-list">
                                <div class="tr-hotel-img">
                                    <a href="javascript:void(0):">
                                        @if ($nearbhotel->hotel_id != '')
                                            <img src="https://photo.hotellook.com/image_v2/limit/h{{ $nearbhotel->hotel_id }}_1/376/229.jpg"
                                                alt="NearBy Hotel" loading="lazy">
                                        @else
                                ...
                                <div class="tr-hotel-deatils">
                                    ...
                                    <div class="tr-like-review">
                                        <span class="tr-likes"><svg width="21" height="21"
                                    ...
                                    @if ($nearbhotel->pricefrom != '')
                                        <div class="tr-price-section"><span>${{ $nearbhotel->pricefrom }}</span>
                                            /night</div>
                                    @endif
                                </div>
                            </div>
                        ...
                        @php
                            ...
                            foreach ($aliasTerms as $alias) {
                                $formattedAliases[] =
                                    ...
                                    url()->current() .
                                    '?alias=' .
                                    urlencode(trim($alias)) .
                                    '" class="tr-alias-name" style="font-size: 18px; margin-right: 5px;">' .
                                    trim($alias) .
                                    '</a>';
                            }
                        @endphp
                        ...
                        @php
                            ...
                            foreach ($searchTerms as $search) {
                                $formattedTerms[] =
                                    ...
                                    url()->current() .
                                    '?search=' .
                                    urlencode(trim($search)) .
                                    '" class="tr-search-name" style="font-size: 18px; margin-right: 5px;">' .
                                    trim($search) .
                                    '</a>';
                            }
                        @endphp
                                        ...
                                        @if (isset($amenity['name']) && isset($amenity['url']) && isset($amenity['code']))
                                            <div class="tr-hotel-list">
                                                <div class="tr-hotel-name">
                                                    <a href="{{ $amenity['url'] }}"
                                                        title="Hotels with {{ $amenity['name'] }}" target="_blank"
                                                        rel="noopener">Hotels with {{ $amenity['name'] }}</a>
                                                </div>
                                            </div>
                                        @endif
                                    @endforeach
                            ...
                            <div class="tr-city-wise-hotel-list">
                                ...
                                <div class="tr-hotel-lists">
                                    @foreach ($fiveStarHotels as $hotel)
                                        <div class="tr-hotel-list">
                                            <div class="tr-hotel-name">
                                                <a href="{{ $hotel['url'] }}" title="{{ $hotel['name'] }}"
                                                    target="_blank" rel="noopener">{{ $hotel['name'] }}</a>
                                            </div>
                                            ...
                                            <div class="tr-hotel-list">
                                                <div class="tr-hotel-name">
                                                    <a href="{{ $neighborhood['url'] }}"
                                                        title="Hotels in {{ $neighborhood['name'] }}"
                                                        target="_blank" rel="noopener">Hotels in
                                                        {{ $neighborhood['name'] }}</a>
                                                </div>
                                            </div>
                                        @endif
                    ...
                    <div class="tr-city-wise-hotel-lists">
                                    ...
                                    @foreach ($popularSections as $section)
                                        @if (isset($section['name']) && isset($section['url']) && isset($section['code']))
                                            <div class="tr-hotel-list">
                                                <div class="tr-hotel-name">
                                                    <a href="{{ $section['url'] }}"
                                                        title="Hotels in {{ $section['name'] }}" target="_blank"
                                                        rel="noopener">Hotels in {{ $section['name'] }}</a>
                                                </div>
                                            </div>
                                        @endif
                                    @endforeach
                        ...
                        @if (isset($nearbyattractions) && count($nearbyattractions) > 0)
                            <div class="tr-city-wise-hotel-list">
                                ...
                                <div class="tr-hotel-lists">
                                    @foreach ($nearbyattractions as $sight)
                                        @if (isset($sight['name']) && isset($sight['url']) && isset($sight['code']))
                                            <div class="tr-hotel-list">
                                                <div class="tr-hotel-name">
                                                    <a href="{{ $sight['url'] }}"
                                                        title="Hotels near {{ $sight['name'] }}" target="_blank"
                                                        rel="noopener">Hotels near {{ $sight['name'] }}</a>
                                                </div>
                                            </div>
                                        @endif
                                    @endforeach
        ...
        <div class="tr-breadcrumb-section">
            <ul class="tr-breadcrumb">
                <li><a href="https://www.travell.co.in">Travell</a></li>
                @if (!$getcontlink->isEmpty())
                    <li>
                        <a
                            href="{{ route('explore_continent_list', [$getcontlink[0]->contid, $getcontlink[0]->cName]) }}">
                            {{ $getcontlink[0]->cName }}</a>
                    </li>
                    <li>
                        <a
                            href="{{ route('explore_country_list', [$getcontlink[0]->CountryId, $getcontlink[0]->slug]) }}">
                ...
                @if (!empty($searchresult))
                    ...
                    @if (!empty($locationPatent))
                        <?php $locationPatents = array_reverse(
                            $locationPatent
                        ); ?>
                        @foreach ($locationPatents as $location)
                            <li>
                    ...
                    @endif
                    @if (!$getlocationexp->isEmpty())
                        <li><a href="{{ route('search.results', [$getlocationexp[0]->slugid . '-' . strtolower($getlocationexp[0]->Slug)]) }}"
                    ...
                    <li><a href="@if (isset($getlocationexp) && !empty($getlocationexp) && isset($getlocationexp[0]) && isset($getlocationexp[0]->Slug)) {{ route('hotel.list', [$firstId . '-' . $getlocationexp[0]->Slug]) }} @endif"
                            target="_blank">
                            @if (isset($getlocationexp) && !empty($getlocationexp) && isset($getlocationexp[0]) && isset($getlocationexp[0]->Name))
                    ...
                    <li>
                        @if (isset($searchresult) && !empty($searchresult) && isset($searchresult[0]) && isset($searchresult[0]->name))
                            {{ $searchresult[0]->name }}
                        @endif
                    </li>
                @endif
                ...
                <div class="tr-share-infos">
                    <div class="tr-hotel-img">
                        @if (!empty($searchresult[0]->hotelid) && isset($images[$searchresult[0]->hotelid][0]))
                            <img
                                src="https://photo.hotellook.com/image_v2/limit/{{ $images[$searchresult[0]->hotelid][0] }}/628/567.auto">
                        @else
                            <img src="{{ asset('/images/Hotel lobby-image.png') }}">
                        @endif
                    </div>
                    <div class="tr-share-details">
                        ...
                        <span class="tr-rating">
                            @if (!empty($searchresult[0]->rating))
                                <?php
                                $rating = (float) $searchresult[0]->rating;
                                $formattedRating = round($rating, 2);
...
?>
            ...
            <div class="tr-write-review-content">
                <div class="tr-hotel-reviews-details">
                    <div class="tr-main-image">
                        <img src="{{ asset('/frontend/hotel-detail/images/no-data-place-image.png') }}"
                            alt="img" class="d-block w-100" height="200" width="482" loading="lazy">
                    </div>
                    <div class="tr-hotel-info">
                ...
                <form id="highlight-review" class="add_highlight_review">
                    ...
                    <div class="tr-review-msg">
                        ...
                        <textarea class="form-control" id="highlight-review-text" placeholder="Type your highlight review here"
                            name="highlight_review"></textarea>
                    ...
                    <div class="tr-review-image">
                        ...
                        <div class="tr-file-upload">
                            <div class="tr-image-upload-wrap">
                                <input class="tr-file-upload-input" type='file' id="highlight-files"
                                    name="files[]" onchange="readHighlightURL(this);" accept="image/*" multiple />
                            ...
                            <div class="tr-file-upload-content">
                                ...
                                <div class="image-title-wrap">
                                    <button type="button" onclick="removeHighlightUpload()"
                                        class="tr-remove-image">
            ...
            <div class="tr-galleries-section" id="galleryOutdoor">
                ...
                <div class="tr-gallery-category ">
                    ...
                    @if ($hotelid != '')
                        <?php
                        $hoteid = $hotelid;
                        $imgcount = isset($images[$hoteid])
                            ? count($images[$hoteid])
                            : 0;
                        ?>
                        <ul>
                            @if (isset($images[$hoteid]) && !empty($images[$hoteid]))
                                @php $imageCounter = 1; @endphp
                                @foreach($images[$hoteid] as $image)
                                    @if (!empty($image))
                                        <li data-bs-toggle="modal" data-bs-target="#gallerySliderModal" class="">
                                            <img src="https://photo.hotellook.com/image_v2/limit/{{ $image }}/1000/1000.auto"
                                                alt="Outdoor Pictures {{ $imageCounter }}">
                                        </li>
                ...
                <div class="tr-hotel-reviews-details">
                    <div class="tr-main-image">
                        @if (!empty($searchresult[0]->hotelid))
                            @if (isset($images[$searchresult[0]->hotelid][0]) && $images[$searchresult[0]->hotelid][0] != '')
                                <img src="https://photo.hotellook.com/image_v2/limit/{{ $images[$searchresult[0]->hotelid][0] }}/628/567.auto"
                                    height="50" class="h-100" alt="Room Image">
                            @else
                ...
                <form id="addReview">
                    <div class="tr-overall-rating">
                        ...
                        <div class="tr-rating">
                            ...
                            <div class="tr-stars star-exp">
                                <input type="radio" data-rating="1" class="star" name="expStar"
                                    id="expStar5" value="5"><label for="expStar5"></label>
                                <input type="radio" data-rating="2" class="star" name="expStar"
                                    id="expStar4" value="4"><label for="expStar4"></label>
                                <input type="radio" data-rating="3" class="star" name="expStar"
                                    id="expStar3" value="3"><label for="expStar3"></label>
                                <input type="radio" data-rating="4" class="star" name="expStar"
                                    id="expStar2" value="2"><label for="expStar2"></label>
                                <input type="radio" data-rating="5" class="star" name="expStar"
                                    id="expStar1" value="1"><label for="expStar1"></label>
                            </div>
                        </div>
                    ...
                    <div class="tr-review-types">
                        <div class="tr-rating">
                            ...
                            <div class="tr-stars star-cleanliness">
                                <input type="radio" class="star" data-rating="1" name="cleanlinessStar"
                                    id="cleanlinessStar5" value="5"><label for="cleanlinessStar5"></label>
                                <input type="radio" class="star" data-rating="2" name="cleanlinessStar"
                                    id="cleanlinessStar4" value="4"><label for="cleanlinessStar4"></label>
                                <input type="radio" class="star" data-rating="3" name="cleanlinessStar"
                                    id="cleanlinessStar3" value="3"><label for="cleanlinessStar3"></label>
                                <input type="radio" class="star" data-rating="4" name="cleanlinessStar"
                                    id="cleanlinessStar2" value="2"><label for="cleanlinessStar2"></label>
                                <input type="radio" class="star" data-rating="5" name="cleanlinessStar"
                                    id="cleanlinessStar1" value="1"><label for="cleanlinessStar1"></label>
                            </div>
                        ...
                        <div class="tr-rating">
                            ...
                            <div class="tr-stars star-location">
                                <input type="radio" class="star" data-rating="1" name="locationStar"
                                    id="locationStar5" value="5"><label for="locationStar5"></label>
                                <input type="radio" class="star" data-rating="2" name="locationStar"
                                    id="locationStar4" value="4"><label for="locationStar4"></label>
                                <input type="radio" class="star" data-rating="3" name="locationStar"
                                    id="locationStar3" value="3"><label for="locationStar3"></label>
                                <input type="radio" class="star" data-rating="4" name="locationStar"
                                    id="locationStar2" value="2"><label for="locationStar2"></label>
                                <input type="radio" class="star" data-rating="5" name="locationStar"
                                    id="locationStar1" value="1"><label for="locationStar1"></label>
                            </div>
                    ...
                    <div class="tr-review-types">
                        <div class="tr-rating star-service">
                            <label>Service :</label>
                            <div class="tr-stars">
                                <input type="radio" class="star" data-rating="1" name="serviceStar"
                                    id="serviceStar5" value="5"><label for="serviceStar5"></label>
                                <input type="radio" class="star" data-rating="2" name="serviceStar"
                                    id="serviceStar4" value="4"><label for="serviceStar4"></label>
                                <input type="radio" class="star" data-rating="3" name="serviceStar"
                                    id="serviceStar3" value="3"><label for="serviceStar3"></label>
                                <input type="radio" class="star" data-rating="4" name="serviceStar"
                                    id="serviceStar2" value="2"><label for="serviceStar2"></label>
                                <input type="radio" class="star" data-rating="5" name="serviceStar"
                                    id="serviceStar1" value="1"><label for="serviceStar1"></label>
                            </div>
                        ...
                        <div class="tr-rating">
                            ...
                            <div class="tr-stars star-value">
                                <input type="radio" class="star" data-rating="1" name="valueStar"
                                    id="valueStar5" value="5"><label for="valueStar5"></label>
                                <input type="radio" class="star" data-rating="2" name="valueStar"
                                    id="valueStar4" value="4"><label for="valueStar4"></label>
                                <input type="radio" class="star" data-rating="3" name="valueStar"
                                    id="valueStar3" value="3"><label for="valueStar3"></label>
                                <input type="radio" class="star" data-rating="4" name="valueStar"
                                    id="valueStar2" value="2"><label for="valueStar2"></label>
                                <input type="radio" class="star" data-rating="5" name="valueStar"
                                    id="valueStar1" value="1"><label for="valueStar1"></label>
                            </div>
                    ...
                    <div class="tr-review-image">
                        ...
                        <div class="tr-file-upload">
                            ...
                            <div class="tr-image-upload-wrap">
                                <input class="tr-file-upload-input" type='file' onchange="readURL(this);"
                                    accept="image/*" id="files" name="files[]" />
                                <div class="tr-drag-image">
                                    <svg width="17" height="16" viewBox="0 0 17 16" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <g clip-path="url(#clip0_441_7517)">
 ...
 <link rel="stylesheet" href="{{ asset('/css/style.css') }}" >
    ...
    <link rel="stylesheet" href="{{ asset('/css/hotel-css/autoComplete.min.css') }}" media="print" onload="this.media='all'; this.onload=null;">
    <link rel="stylesheet" href="{{ asset('/css/custom.css') }}" media="print" onload="this.media='all'; this.onload=null;">
    <link rel="stylesheet" href="{{ asset('/css/hotel-css/t-datepicker.min.css') }}" media="print" onload="this.media='all'; this.onload=null;">
    <link rel="stylesheet" href="{{ asset('/frontend/hotel-detail/css/calendar.css') }}" media="print" onload="this.media='all'; this.onload=null;">
    <link rel="stylesheet" href="{{ asset('/css/map_leaflet.css') }}" media="print" onload="this.media='all'; this.onload=null;">
    <link rel="stylesheet" href="{{ asset('/frontend/hotel-detail/css/custom.css') }}" media="print" onload="this.media='all'; this.onload=null;">
    <link rel="stylesheet" href="{{ asset('css/cookie-consent.css') }}" media="print" onload="this.media='all'; this.onload=null;">
    ...
    <script src="{{ asset('/frontend/hotel-detail/js/jquery-ui-datepicker.min.js') }}" defer></script>
    <script src="{{ asset('/frontend/hotel-detail/js/jquery.min.js') }}" defer></script>
    <script src="{{ asset('/js/custom.js') }}" defer></script>
    <script src="{{ asset('/frontend/hotel-detail/js/common.js') }}" defer></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.7.1/leaflet.js" defer></script>
    <script src="{{ asset('/js/map_leaflet.js') }}" defer></script>
    <script src="{{ asset('/js/hotelDetails.js') }}" defer></script>
    <script src="{{ asset('/frontend/hotel-detail/js/leaflet-simple-map-screenshoter.js') }}" defer></script>
    <script src="{{ asset('/frontend/hotel-detail/js/FileSaver.js') }}" defer></script>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        var overpassApiUrl =
            'https://overpass-api.de/api/interpreter?data=[out:json];(way["highway"];way["natural"="water"];relation["waterway"="riverbank"];>;);out body;';
        ...
        fetch(overpassApiUrl)
            ...
            .then(data => {
                ...
                L.geoJSON(geoJsonData, {
                    filter: function(feature) {
                        return feature.properties.natural === 'water' || feature.properties
                            .waterway === 'riverbank';
                    },
                    style: function() {
                        return {
...
<script>
    ...
    function updateShareLinks() {
        ...
        document.getElementById("whatsappShare").href =
            `https://api.whatsapp.com/send?text=${encodeURIComponent("Check this out: " + currentUrl)}`;
        document.getElementById("facebookShare").href =
            `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(currentUrl)}`;
        document.getElementById("twitterShare").href =
            `https://twitter.com/intent/tweet?url=${encodeURIComponent(currentUrl)}&text=${encodeURIComponent("Check this out!")}`;
...
<script>
    ...
    $(document).ready(function() {
        ...
        $("#question").on('input', function() {
            ...
            $(".formtext").text((300 - charCount) + " Characters left");
        });
        ...
        $("#submitQuestion").click(function() {
            ...
            if (questionText === "") {
                $("#questionResponse").removeClass("alert-success").addClass("alert-danger").text("Please enter your question.").show();
                return;
            }

            // Show loading state
            $("#submitQuestion").prop('disabled', true).text('Submitting...');

            $.ajax({
                url: '{{ route("hotel.submit.question") }}',
                type: 'POST',
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        $("#questionResponse").removeClass("alert-danger").addClass("alert-success").text(response.message).show();
                        // Clear the form
                        $("#question").val('');
                        $("#charCount").text("0/300");
                        $(".formtext").text("300 Characters left");

                        // Reset button after 2 seconds
                        setTimeout(function() {
                            $("#submitQuestion").prop('disabled', false).text('Ask a question');
                        }, 2000);
                    } else {
                        $("#questionResponse").removeClass("alert-success").addClass("alert-danger").text(response.message).show();
                        $("#submitQuestion").prop('disabled', false).text('Ask a question');
                    }
                },
                error: function(xhr) {
                    ...
                    $("#questionResponse").removeClass("alert-success").addClass("alert-danger").html(errorMessage).show();
                    $("#submitQuestion").prop('disabled', false).text('Ask a question');
                }
            });
        });
    ...
    function toggleAnswer(element) {
        var $parent = $(element).closest('.info__details__title_body');
        var $truncated = $parent.find('.truncated-answer');
        var $full = $parent.find('.full-answer');
    ...
    function showSpecificFaq(faqId) {
        // Hide all FAQ items in the sidebar initially
        $('.info__main').hide();

        // Show only the FAQ with the matching ID
        $('.info__main').each(function() {
            var currentFaqId = $(this).data('faq-id');
            if (currentFaqId == faqId) {
                $(this).show();
            }
        });
    }

    // Add a direct script to fix the View All button functionality
    document.addEventListener('DOMContentLoaded', function() {
        console.log('DOM Content Loaded. Initial storedSearchValue:', window.storedSearchValue, 'isOpeningViaSpecificFaq:', window.isOpeningViaSpecificFaq);
        // Store the original showSpecificFaq function
        var originalShowSpecificFaq = showSpecificFaq;

        // Variable to store the search input's value temporarily
        window.storedSearchValue = '';
        // Flag to indicate if the offcanvas is being opened by a specific FAQ click
        window.isOpeningViaSpecificFaq = false;

        // Override the showSpecificFaq function to track the last action
        window.showSpecificFaq = function(faqId) {
            console.log('showSpecificFaq called. faqId:', faqId);
            // Set flag to indicate opening via specific FAQ
            window.isOpeningViaSpecificFaq = true;
            console.log('After showSpecificFaq: isOpeningViaSpecificFaq set to true. storedSearchValue:', window.storedSearchValue);
            // Call the original function
            originalShowSpecificFaq(faqId);
        };

        // Add click handler for the View All button
        document.querySelector('.viewAll').addEventListener('click', function() {
            console.log('View All button clicked.');
            // Set flag to indicate NOT opening via specific FAQ
            window.isOpeningViaSpecificFaq = false;
            console.log('After View All click: isOpeningViaSpecificFaq set to false. storedSearchValue:', window.storedSearchValue);

            // The search input will be cleared in the 'shown.bs.offcanvas' event if opened by 'View All'.
            console.log('View All button clicked. Clearing logic moved to shown.bs.offcanvas.');

            // Show all FAQs immediately
            document.querySelectorAll('.info__main').forEach(function(el) {
        ...
        var faqSidebarSearchElement = document.getElementById('faqSidebarSearch');

        if (faqSidebarSearchElement) {
            // Event listener for when the offcanvas has completed its opening transition
            faqSidebarSearchElement.addEventListener('shown.bs.offcanvas', function () {
                var faqSearchInput = document.getElementById('faqSearchInput');
                console.log('Offcanvas SHOWN event. isOpeningViaSpecificFaq:', window.isOpeningViaSpecificFaq, 'storedSearchValue:', window.storedSearchValue);
                if (faqSearchInput) {
                    if (window.isOpeningViaSpecificFaq) {
                        // If opened by specific FAQ, restore the stored value
                        faqSearchInput.value = window.storedSearchValue;
                        console.log('Restoring search input value to:', faqSearchInput.value);
                    } else {
                        // If opened by 'View All', ensure it's blank
                        faqSearchInput.value = '';
                        console.log('Clearing search input value for View All.');
                    }
                }
            });

            // Event listener for when the offcanvas is hidden (closed)
            faqSidebarSearchElement.addEventListener('hidden.bs.offcanvas', function () {
                var faqSearchInput = document.getElementById('faqSearchInput');
                console.log('Offcanvas HIDDEN event. Current input value BEFORE storing/clearing:', faqSearchInput ? faqSearchInput.value : 'N/A');
                if (faqSearchInput) {
                    // Only store the value if it's not empty, to preserve previous search terms
                    if (faqSearchInput.value !== '') {
                        window.storedSearchValue = faqSearchInput.value;
                        console.log('Stored search value UPDATED to:', window.storedSearchValue);
                    } else {
                        console.log('Stored search value RETAINED as:', window.storedSearchValue, '(input was empty)');
                    }
                    // Always clear the input visually when closed
                    faqSearchInput.value = '';
                    console.log('Search input visually cleared. Final storedSearchValue:', window.storedSearchValue);
                }
            });
        }
    });

    // Handle FAQ like/dislike functionality
    function voteFaq(element, faqId, action) {
        event.preventDefault(); // Prevent default link behavior
        console.log('voteFaq called with:', { element, faqId, action });

        // Check if jQuery is available
        if (typeof $ === 'undefined') {
            console.error('jQuery is not loaded!');
            alert('Error: jQuery is not loaded. Please refresh the page and try again.');
            return;
        }

        try {
            // Get the count element - always use like-count since we only have one counter now
            const $element = $(element);
            const $parentContainer = $element.closest('.like__button');
            const $countElement = $parentContainer.find('.like-count');

            // Add a temporary visual feedback immediately
            $element.addClass('voting');

            console.log('Sending AJAX request to:', '{{ route("hotel.faq.vote") }}');
            console.log('With data:', { faq_id: faqId, action: action });

            // Send AJAX request to update like count
            $.ajax({
                url: '{{ route("hotel.faq.vote") }}',
                type: 'POST',
                data: {
                    faq_id: faqId,
                    action: action,
                ...
                dataType: 'json',
                beforeSend: function() {
                    // Disable both buttons during the request to prevent double-clicks
                    $parentContainer.find('.like-btn, .dislike-btn').css('pointer-events', 'none');
                },
                success: function(data) {
                    console.log('AJAX success response:', data);
                    if (data.success) {
                        // Update the count display (always update the like-count)
                        $countElement.text(data.count);

                        // Add a visual feedback class
                        $element.removeClass('voting').addClass('voted');

                        // Disable both buttons after voting
                        $parentContainer.find('.like-btn, .dislike-btn').addClass('disabled');

                        // Show a brief success message in the UI instead of an alert
                        const message = action === 'like' ? 'Thank you for your positive feedback!' : 'Thank you for your feedback!';
                        const $feedback = $('<div class="vote-feedback">' + message + '</div>');
                        $parentContainer.append($feedback);
                        setTimeout(function() {
            ...
            console.error('Exception in voteFaq:', e);
...
<script>
    $(document).ready(function() {
        // Variables to store FAQs data
        let allFaqs = [];

        // Initialize by collecting all FAQ data
        $('.info__main').each(function() {
            const $faq = $(this);
            const faqId = $faq.data('faq-id');
            const question = $faq.find('.infoSection p').text().trim();
            const answer = $faq.find('.info__details__title_body').text().trim();

            // Get tags from data attribute if available, otherwise use empty array
            const tagsString = $faq.data('tags') || '';
        ...
        $('#faqSearchInput').on('keyup', function() {
            ...
            if (searchTerm.length < 2) {
                // Show all FAQs if search term is too short
                $('.info__main').show();
                $('#searchResults').hide();
                return;
            }
        ...
        $('#faqSearchButton').on('click', function() {
            const searchTerm = $('#faqSearchInput').val().trim().toLowerCase();

            if (searchTerm.length < 2) {
                return;
            }

            // If search term is entered, perform server-side search
            performServerSearch(searchTerm);
        });

        // Function to update search results in UI
        function updateSearchResults(results, searchTerm) {
            if (results.length === 0) {
                // If no results, show all FAQs
                $('.info__main').show();

                // Show a message that no results were found
                $('#searchResults').html('<p>No results found for "' + searchTerm + '"</p>').show();
                return;
            }
        ...
        function performServerSearch(searchTerm) {
            $.ajax({
                url: '{{ route("hotel.faq.search") }}',
                type: 'GET',
                data: {
                    hotel_id: {{ $searchresults[0]->id }},
                    query: searchTerm
                },
                dataType: 'json',
                beforeSend: function() {
                ...
                success: function(data) {
                    if (data.success && data.faqs.length > 0) {
                        // Process results
                        const results = data.faqs;

                        // Hide all FAQs first
                        $('.info__main').hide();

                        // Show only matching FAQs by ID
                        results.forEach(faq => {
                            $('.info__main[data-faq-id="' + faq.id + '"]').show();
                        });

                        // Hide search results container
                        $('#searchResults').hide();
                    } else {
                        // If no results or error, show message
                        $('#searchResults').html('<p>No results found for "' + searchTerm + '"</p>').show();
                    }
                },
                error: function() {
