...
<?php
// Proper deduplication logic at the start
$uniqueHotels = [];
$displayResults = [];
$perPage = 30; // Set your desired items per page

foreach ($searchresults as $result) {
    if (!in_array($result->hotelid, $uniqueHotels)) {
        $uniqueHotels[] = $result->hotelid;
        $displayResults[] = $result;
    }
    
    // Stop when we reach the per-page limit
    if (count($displayResults) >= $perPage) {
        break;
    }
}

$displayCount = count($displayResults);
$totalCount = $searchresults->total(); // Total available results
?>
@if(!empty($displayResults))
<?php $a = 1;?>
<span class="d-none page_type">getlist</span>
@foreach($displayResults as $searchresult)
<?php
...
?>
<div class="tr-hotel-deatils clickable-hotel-card" data-id="{{ $searchresult->id }}" data-url="{{ $hotel_url }}">
...
<div class="tr-hotel-deatil">
    <a href="{{ $hotel_url }}" class="hotel-details-link"></a>
    <div class="tr-heading-with-rating">
      <h2>
  <a href="{{ $hotel_url }}" class="hotel-name-link" onclick="event.stopPropagation();">
...
<div class="tr-hotel-facilities">
    <?php
        // Define an array to map each selected amenity to its specific icon path (same as main file)
        $amenityIconPaths = [
        ...
        // Selected amenity names we support (keys from icon map)
        $selectedAmenities = array_keys($amenityIconPaths);

        $amenities = [];
        if (!empty($searchresult->amenity_info)) {
            // Normalize and split the amenity_info string
            $rawAmenityInfo = (string) $searchresult->amenity_info;
            $normalized = preg_replace('/\s*\|\|\|\s*/u', '|||', $rawAmenityInfo);
            $amenityData = explode('|||', $normalized);
            ...
            foreach ($amenityData as $item) {
                if (strpos($item, '|') !== false) {
                    ...
                    $normalizedName = null;
                    if (preg_match('/laundry/', $lower)) { $normalizedName = 'Laundry'; }
                    elseif (preg_match('/wifi|wi-fi|wi fi|internet/', $lower)) { $normalizedName = 'Wi-Fi'; }
                    elseif (preg_match('/parking/', $lower)) { $normalizedName = 'Parking'; }
                    elseif (preg_match('/spa/', $lower)) { $normalizedName = 'Spa'; }
                    elseif (preg_match('/gym|fitness/', $lower)) { $normalizedName = 'Gym'; }
                    elseif (preg_match('/pool|swimming/', $lower)) { $normalizedName = 'Pool'; }
                    elseif (preg_match('/bar/', $lower)) { $normalizedName = 'Bar'; }
                    elseif (preg_match('/restaurant|food|dining/', $lower)) { $normalizedName = 'Food'; }
                    elseif (preg_match('/a\/c|air conditioning|air conditioner|climate/', $lower)) { $normalizedName = 'A/C'; }
                    elseif (preg_match('/smoke/', $lower)) { $normalizedName = 'Smoke-free'; }

                    if ($normalizedName && in_array($normalizedName, $selectedAmenities)) {
                        $amenities[] = [
                            'name' => $normalizedName,
                            'available' => $available,
                        ];
            ...
            $uniqueAmenities = array_slice($uniqueAmenities, 0, 5);
        } else {
            $uniqueAmenities = [];
        }
    ?>
  ...
  <div class="tr-hotel-price-section">
    ...
    <div class="tr-hotel-price-lists">
      @if(!$hotels->isEmpty())
        ...
        <button class="tr-more-price tr tr-anchor-btn" onclick="this.textContent = this.textContent === 'View More Offers' ? 'View Less Offers' : 'View More Offers';">View More Offers</button>
        @endif
...
{{-- Update count displays to use: --}}
{{ $displayCount }} of {{ $totalCount }} hotels
...
<script>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    console.log('Initializing hotel card click handlers');
    const hotelCards = document.querySelectorAll('.clickable-hotel-card');
    ...
    hotelCards.forEach(card => {
      ...
      card.addEventListener('click', function(e) {
        ...
        // Prevent click if it was on a link inside the card
        if (e.target.closest('a')) {
          console.log('Click on internal link, not redirecting');
          return;
        }
...
.tr-hotel-deatil a.hotel-details-link {
    display: block;
    position: absolute;
    top: 0;
    left: 0;
