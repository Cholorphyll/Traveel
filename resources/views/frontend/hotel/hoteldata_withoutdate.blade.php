  ...
  <div class="tr-hotel-deatils">
    ...
    <div class="tr-hotel-location">
       @if($searchresult->slugid && $searchresult->NeighborhoodId && ($location_info->Slug ?? $searchresult->slug))
            <a href="https://www.travell.co.in/ho-{{$searchresult->slugid}}-{{$location_info->Slug ?? $searchresult->slug}}-nqx{{$searchresult->NeighborhoodId}}">{{$searchresult->Neighborhood}}</a>
        @else
        ...
        @if(isset($searchresult->calculated_distance))
            ...
            <a href="https://www.travell.co.in/at-{{$sight_info['sight_Location_id']}}-{{$sight_info['sight_id']}}-{{$sight_info['sight_slug']}}">
...
@endif
    ...
    <div class="short-description-content overviewText">
    <ul style="max-height: fit-content;">
        @foreach($mergedOverview as $index => $data)
            <li class="{{ $index < 5 ? 'visible' : '' }}">
                {{ trim($data, " '[]") }}
            </li>
        @endforeach
    </ul>
      ...
      <button type="button" class="custom-read-more readMoreBtn" onclick="toggleContent(this)">
  Read More
</button>
    @endif
...
<script>
  document.addEventListener("DOMContentLoaded", function () {
    const visibleCount = 4; // how many <li> to show initially
  ...
  function toggleContent(button) {
    const content = button.previousElementSibling; // parent .short-description-content
    const listItems = content.querySelectorAll("ul li");
    const isExpanded = content.classList.contains("show-more");

    if (isExpanded) {
      // Collapse → show only 4
      listItems.forEach((li, index) => {
        li.classList.toggle("visible", index < 4);
      });
