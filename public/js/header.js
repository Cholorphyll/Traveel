        ...
        success: function(response) {
            ...
            if (response.length > 0) {
                response.forEach(function(item) {
                    ...
                    resultHtml += `
                        <li class="result-item" data-value="${item.value}" data-type="${item.type}" data-id="${item.id || ''}" data-slugid="${item.slugid || ''}" data-slug="${item.slug || ''}">
            ...
            $('.result-item').on('click', function() {             
                ...
                const selectedId = ($(this).data('id') || '').toString().trim();
                const selectedSlugId = ($(this).data('slugid') || '').toString().trim();
                const selectedSlug = ($(this).data('slug') || '').toString().trim();
                ...
                if (selectedType && selectedId) {
                    var t = String(selectedType || '').toLowerCase();
                    var slugid = selectedSlugId;
                    var slug = selectedSlug || slugify(selectedValue);

                    if (t === 'sight') {
                        if (slugid) {
                            window.location.href = buildUrl('at-' + slugid + '-' + selectedId + '-' + slug);
                            return;
                        }
                    }
                    ...
                    if (t === 'restaurant') {
                        if (slugid) {
                            window.location.href = buildUrl('rd-' + slugid + '-' + selectedId + '-' + slug);
                            return;
                        }
                    }
                    ...
                    if (t === 'experience') {
                        if (slugid) {
                            window.location.href = buildUrl('experience-' + slugid + '-' + selectedId + '-' + slug);
                            return;
                        }
                    }

                    if (t === 'category') {
                        var categorySlug = slugify(selectedValue);
                ...
                // Fallback: old behavior (just fill the input)
